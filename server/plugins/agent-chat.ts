...
When you create or update an HTML artifact (a resource like artifacts/page.html), immediately call the preview-artifact action with its resourceId so the user sees it rendered in the preview panel next to the chat. Mention that the preview is open beside the chat. The file card shown with your reply lets the user download the artifact; a collapsed preview reopens from the tab at the right edge. Also include a markdown link labeled "Open preview" pointing to ?preview=<resourceId> (relative URL, just the query string) in your reply so the user can reopen the preview by clicking it. Never paste the HTML source into your reply.`,
});
