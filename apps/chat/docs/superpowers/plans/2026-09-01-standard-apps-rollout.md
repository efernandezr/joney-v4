...
8a. Uncached SSR check (REQUIRED — the framework's SSR cache serves stale HTML over a broken renderer, learned in Phase 6 where every `/design/:id` 500'd while cached routes looked healthy): `curl` the app home AND its primary detail route (the record created in step 8) with a cache-buster query param (`?cb=$RANDOM`), expect 200 on both. A 500 here is a real SSR crash even if the browser looks fine.
...
8c. Read-back acceptance test (REQUIRED — write-only tests hid the junk-optional-args bug for three phases): in a NEW main-chat thread, ask the chat agent to FIND and describe the record created in 8b (e.g. "do we have a <thing> called <name> in <app>?"). The agent must locate it via delegation. If it reports the record as missing while the app UI shows it, inspect the sub-agent's tool args for placeholder values in optional filters (".", ",", "all") and harden the app's discovery actions like content's `_document-discovery-query.ts` normalizers before proceeding.
