...
describe("migrate-content-database-rows", () => {
  ...
  // Skipped in the scaffolded workspace: vi.spyOn cannot redefine ESM
  // namespace exports of the PUBLISHED @agent-native/core dist (works only
  // against monorepo sources). Template-repo contract test.
  it.skip("fails closed on shared SQLite before flushing but permits a terminal replay", async () => {
    const seed = await fixture(1);
    const input = plan(seed);
