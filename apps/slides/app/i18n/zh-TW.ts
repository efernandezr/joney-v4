import { creativeContextMessagesByLocale } from "@agent-native/creative-context/messages";

const messages = {
  creativeContext: creativeContextMessagesByLocale["zh-TW"],
  root: {
    commandPresentations: "簡報",
    searchDecks: "搜尋幻燈片",
    commandAppearance: "外觀",
    toggleTheme: "切換主題",
  },
  header: {
    settings: "設定",
    decks: "幻燈片",
    designSystems: "設計系統",
    team: "團隊",
    extensions: "擴充功能",
    deck: "幻燈片",
    tool: "工具",
    slides: "幻燈片",
  },
  navigation: {
    settings: "設定",
    brand: "幻燈片",
    decks: "幻燈片",
    designSystems: "設計系統",
    team: "團隊",
  },
  settings: {
    title: "設定",
    description: "此應用的語言和工作區偏好設定。",
    emailNotifications: "郵件通知",
    emailNotificationsDescription:
      "當有人評論你的簡報或在討論串中回覆時，收到郵件通知。",
    saveFailed: "儲存失敗",
    languageTitle: "語言",
    languageDescription: "選取介面語言。此偏好會儲存到你的帳戶。",
    languageLabel: "介面語言",
    workspaceTitle: "工作區",
    workspaceDescription: "管理團隊成員、組織存取權限和共用工作區偏好。",
    openTeamSettings: "開啟團隊設定",
    openResourceSettings: "開啟資源設定",
    agentTitle: "管理代理",
    agentDescription: "管理代理的模型、API 金鑰、自動化、語音和其他控制項。",
    openAgentSettings: "管理代理",
  },
  agent: {
    emptyState: "可以詢問我任何關於簡報的問題",
    thisSlide: "此投影片",
    currentSelection: "目前選取項目",
    suggestionPitch: "根據這份檔案製作 10 頁面推介幻燈片",
    suggestionBrand: "將我們的品牌應用到這份幻燈片",
    suggestionHero: "為這頁面幻燈片生成主視覺圖片",
  },
  sidebar: {
    openNavigation: "開啟導覽",
    expandSidebar: "展開側邊欄",
    collapseSidebar: "折疊側邊欄",
  },
  notFound: {
    message: "此頁面尚不存在。",
    backToDecks: "返回簡報",
  },
  raw: {
    deckOptions: "簡報選項",
    generatingImage: "正在產生圖片...",
    noAssetsYet: "還沒有素材。",
    assetUploadFailed: "部分上傳失敗",
    assetDeleteFailed: "刪除素材失敗",
    contentMarkdown: "內容 (Markdown)",
    speakerNotes: "演講者備註",
    speakerNotesForSlide: "演講者備註 — 幻燈片 {{index}} / {{count}}",
    writeContent: "用 Markdown 編寫幻燈片內容...",
    addSpeakerNotes: "新增演講者備註...",
    zoomOut: "縮小",
    zoomIn: "放大",
    fitSlideToScreen: "使幻燈片適應螢幕",
    fitToScreen: "適應螢幕",
    exitExcalidrawCanvas: "退出繪圖畫布",
    agentWorking: "代理正在工作",
    openChat: "開啟聊天",
    cloudAuthToken: "認證權杖",
    cloudSetupSteps: "設定步驟",
    cloudConnectedReloading: "已連線。正在重新載入...",
    cloudConnectionFailed: "連線失敗",
    cloudTestConnect: "測試並連線",
    cloudSqliteEdge: "邊緣 SQLite",
    cloudServerlessPostgres: "無伺服器 Postgres",
    cloudOpenSourceFirebaseAlternative: "Firebase 的開放原始碼替代方案",
    cloudCloudflareSqliteEdge: "Cloudflare 邊緣 SQLite",
    designSlidePreview: "幻燈片預覽",
    designSlideTitle: "幻燈片標題",
    designSupportingText:
      "這裡會顯示將品牌樣式一致套用到所有幻燈片的輔助文字。",
    pasteUrl: "貼上 URL",
    removeLink: "移除連結",
    firstSlidesTitle: "正在準備第一批幻燈片",
    firstSlidesDescription: "每張幻燈片送達後，簡報會開始顯示在這裡。",
    googleOAuthNotConfigured: "此部署尚未設定 Google OAuth。",
    googlePickerNeedsKeys:
      "Google Picker 需要 GOOGLE_PICKER_API_KEY 和 GOOGLE_PICKER_APP_ID。",
    imageUploadFailed: "圖片上傳失敗",
    imageUploadNeedsBuilder:
      "請從代理編寫器的模型選單連接 Builder.io，以便將圖片上傳到投影片。即使沒有提供者，將圖片拖到空白畫布上仍可傳送給代理。",
    sentToAgent: "已傳送給代理",
    imageUploadGenericError: "上傳這張圖片時發生問題。",
    uploading: "正在上傳…",
    imagePromptTitle: "你想怎麼處理這張圖片？",
    imagePromptPlaceholder:
      "例如：作為標題投影片的主視覺。或：替換第 3 張投影片的人像照片。",
    imageSearchTitle: "搜尋圖片",
    searchImagesPlaceholder: "搜尋圖片...",
    searchForLogosImagesIcons: "搜尋徽標、圖片、圖示...",
    search: "搜尋",
    searchFailed: "搜尋失敗",
    logoSearchTitle: "徽標搜尋",
    searchCompanyLogo: "搜尋公司以尋找徽標",
    searchCompanyPlaceholder: "搜尋公司名稱（例如：Intuit）",
    forDarkBg: "適用深色背景",
    onDark: "暗色上",
    symbolIcon: "符號 / 圖示",
    symbolDark: "深色符號",
    importFileTypes: "匯入 PPTX、DOCX、PDF",
    mermaidConvertPrompt:
      "是否要將這個 Mermaid 圖轉換為可編輯的 Excalidraw 圖面？",
    uploadAttachedFailed: "無法上傳附件。",
    uploadFailed: "上傳失敗",
    doubleClickEdit: "按兩下文字以編輯",
    dragToMove: "拖曳以移動",
    aiEditing: "AI 正在編輯",
    startTypingCommands: "開始輸入…或輸入 / 使用命令",
    pinDropHint: "點選任意位置以新增評論圖釘",
    editSlide: "編輯幻燈片",
    escExit: "按 Esc 結束",
    tellAgentChange: "告訴代理要變更什麼…",
    drawClearAll: "全部清除",
    drawExitMode: "結束繪圖模式",
    tellAgentDo: "輸入希望代理執行的內容…",
    typeAnnotation: "輸入註解…",
    typeAnywhere: "在畫布上的任意位置輸入",
    undoStroke: "復原筆畫",
    clearSelection: "清除選取",
    sendToAgent: "傳送給代理",
    saveReconnect: "重新連線時會儲存變更",
    offline: "離線",
    selected: "已選取",
    chooseDesignSystem: "選取設計系統",
    presentationNotFound: "未找到簡報",
    sharedPresentationExpired: "這份共用簡報不存在或已過期。",
    routeNotFoundTitle: "找不到 - Slides",
    routeEditorTitle: "編輯器 — Slides",
    routeDesignSystemsTitle: "設計系統 - Slides",
    routeSettingsTitle: "設定 - Slides",
    routeSharedTitle: "共用簡報",
    routeTeamTitle: "團隊 — Slides",
    slidePreviewTitle: "幻燈片預覽",
    slideUnavailable: "無法使用幻燈片",
    couldNotLoadSlide: "無法載入幻燈片。",
    openInApp: "在應用中開啟",
    teamDescription: "設定團隊，與同事共用簡報。",
  },
  designSystems: {
    new: "新建設計系統",
    setupBrand: "設定你的品牌",
    delete: "刪除",
    cancel: "取消",
    moreActions: "更多操作",
    deleteDialogTitle: "刪除設計系統？",
    deleteDialogDescription:
      "此操作會永久刪除該設計系統。使用它的簡報會保留目前外觀，但不再與其連結。",
    deleteError: "刪除設計系統失敗",
    emptyTitle: "設定你的品牌識別",
    emptyDescription:
      "使用你的品牌顏色、字型和徽標建立設計系統。每個新簡報都會遵循你的視覺識別。",
  },
  editorToolbar: {
    layoutTitle: "標題",
    layoutSection: "分節頁面",
    layoutContent: "內容",
    layoutTwoColumn: "雙欄",
    layoutImage: "圖片",
    layoutStatement: "陳述",
    layoutFullImage: "整頁面圖片",
    layoutBlank: "空白",
    importingFile: "正在匯入檔案",
    readingFile: "正在讀取 {{fileName}}...",
    uploadFailed: "上傳失敗",
    uploadMissingPath: "上傳未傳回檔案路徑",
    importFailed: "匯入失敗",
    importFailedDescription: "匯入此檔案時出了點問題。",
    importComplete: "匯入完成",
    importCompleteSlides_other: "已從 {{fileName}} 匯入 {{count}} 張幻燈片。",
    importCompleteFile: "已從 {{fileName}} 匯入檔案。",
    backToDecks: "返回幻燈片",
    toggleSlideList: "切換幻燈片清單",
    designSystem: "設計系統",
    usingDesignSystem: "正在使用 {{title}}",
    usingLinkedDesignSystem: "正在使用已關聯的設計系統",
    viewOnly: "僅檢視",
    commenterRoleLabel: "評論者",
    commenterRoleDescription: "可以檢視並新增評論",
    slideSettings: "幻燈片設定",
    layout: "布局",
    background: "背景",
    customColor: "自訂顏色",
    media: "媒體",
    generateImage: "產生圖片",
    assetLibrary: "素材庫",
    diagrams: "圖表",
    insertMermaidDiagram: "插入 Mermaid 圖表",
    insertMermaidFailed: "插入圖表失敗",
    excalidrawCanvas: "Excalidraw 畫布",
    convertMermaidToExcalidraw: "將 Mermaid 轉為 Excalidraw",
    convertMermaidFailed: "轉換圖表失敗",
    removeExcalidrawCanvas: "移除 Excalidraw 畫布",
    transition: "過渡",
    transition_instant: "立即",
    transition_fade: "淡入淡出",
    transition_slide: "滑動",
    transition_zoom: "縮放",
    aspectRatio: "寬高比",
    setAspectRatio: "將幻燈片設為 {{ratio}}",
    slideTools: "幻燈片工具",
    elementAnimations: "元素動畫",
    layers: "圖層",
    closeLayers: "關閉圖層面板",
    expandLayer: "展開圖層",
    collapseLayer: "收合圖層",
    tweaks: "微調",
    drawOnSlide: "註解投影片",
    addTextBox: "新增文字方塊",
    shapes: "形狀",
    shapeRectangle: "矩形",
    shapeCircle: "圓形",
    shapeArrow: "箭頭",
    shapeTriangle: "三角形",
    shapeLine: "線",
    pinComments: "固定評論",
    undo: "撤銷",
    undoWithShortcut: "撤銷 ({{shortcut}})",
    redo: "重做",
    redoWithShortcut: "重做 ({{shortcut}})",
    preview: "預覽",
    code: "程式碼",
    comments: "評論",
    editorLink: "編輯器連結",
    editorLinkDescription:
      "在編輯器中開啟幻燈片。有存取權限的人都可以使用此連結。",
    presentationLink: "簡報連結",
    presentationLinkDescription: "直接以全螢幕簡報模式開啟。",
    present: "播放簡報",
    more: "更多",
    importing: "正在匯入...",
    importFile: "匯入檔案",
    savedVersions: "已儲存版本",
    lightTheme: "淺色主題",
    darkTheme: "深色主題",
  },
  editorExport: {
    pptxFailed: "無法生成 PPTX 檔案。",
    exportFailed: "匯出失敗",
    exportPptxError: "匯出為 PPTX 時出了點問題。",
    googleSlidesDownloaded: "已為 Google Slides 下載",
    googleSlidesImportHint:
      "開啟 Google Slides，選取 File -> Import slides，然後選取下載的 .pptx。",
    exportGoogleSlidesError: "匯出到 Google Slides 時出了點問題。",
    htmlFailed: "無法生成 HTML 檔案。",
    exportHtmlError: "匯出為 HTML 時出了點問題。",
    export: "匯出",
    exportAndDuplicate: "匯出和複製",
    shareWithTeam: "與團隊共用...",
    publicShareLink: "公開共用連結...",
    downloadHtml: "下載為 HTML",
    exportPdf: "匯出為 PDF",
    exportPptx: "匯出為 PPTX",
    connectGoogle: "連接 Google",
    openInGoogleSlides: "在 Google Slides 中開啟",
    googleSlidesCreated: "已在 Google Slides 中開啟",
    googleSlidesCreatedHint: "已在你的 Google 雲端硬碟中建立此簡報的副本。",
    duplicateDeck: "複製幻燈片",
  },
  share: {
    createFailed: "無法建立共用連結",
    title: "共用簡報",
    cloudUpgradeDescription:
      "要公開共用簡報，請連線雲端資料庫，讓幻燈片可從任何位置存取。",
    description:
      "為“{{title}}”建立可共用連結。只有此簡報可存取，其他幻燈片不會共用。",
    whatGetsShared: "共用內容：",
    slideContent: "- 幻燈片內容和布局（{{count}} 張幻燈片）",
    presentationView: "- 簡報檢視（全螢幕）",
    whatStaysPrivate: "保持私密：",
    speakerNotes: "- 演講者備註",
    otherPresentations: "- 其他簡報",
    editingAccess: "- 編輯存取權限",
    creatingLink: "正在建立連結...",
    createShareLink: "建立共用連結",
    copyLink: "複製連結",
    copied: "已複製",
    openSharedLink: "開啟共用連結",
    anyoneWithLink: "擁有此連結的任何人都可以檢視此簡報。",
  },
  slideTextMenu: {
    brandColors: "品牌顏色",
    editPalette: "編輯調色板",
    addColor: "新增顏色",
    removeColor: "移除顏色",
    bold: "加粗",
    italic: "斜體",
    strikethrough: "刪除線",
    code: "程式碼",
    heading1: "標題 1",
    heading2: "標題 2",
    heading3: "標題 3",
    bulletList: "專案符號清單",
    orderedList: "編號清單",
    link: "連結",
    pasteUrl: "貼上 URL...",
    apply: "應用",
    cancel: "取消",
    textColor: "文字顏色",
    copyStyleShortcut: "{{shortcut}} 複製樣式",
    pasteStyleShortcut: "{{shortcut}} 貼上樣式",
    comment: "評論",
  },
  slideSlashMenu: {
    blocks: "塊",
    text: "文字",
    plainParagraph: "普通段落",
    heading1: "標題 1",
    largeSlideHeading: "大型幻燈片標題",
    heading2: "標題 2",
    mediumHeading: "中等標題",
    heading3: "標題 3",
    smallHeading: "小標題",
    bulletList: "專案符號清單",
    unorderedList: "無序清單",
    numberedList: "編號清單",
    orderedList: "有序清單",
    quote: "引用",
    blockquote: "塊引用",
  },
  comments: {
    deleteComment: "刪除評論",
    saveCommentFailed: "無法儲存此評論。",
    addCommentPlaceholder: "新增評論...",
    cancel: "取消",
    saving: "正在儲存...",
    comment: "評論",
    saveReplyFailed: "無法儲存此回覆。",
    replyPlaceholder: "回覆...",
    reply: "回覆",
    resolveThread: "解決討論",
    hideReplies: "隱藏回覆",
    replyCount: "{{count}} 條回覆",
    title: "評論",
    addComment: "新增評論",
    close: "關閉",
    hideResolved: "隱藏已解決",
    showResolved: "顯示 {{count}} 條已解決",
    noCommentsYet: "還沒有評論",
    loadFailed: "無法載入評論",
    retry: "重試",
    clickToAddComment: "點選新增評論",
    selectSlideToAdd: "選取幻燈片以新增評論",
  },
  styleInspector: {
    title: "Style",
    close: "關閉樣式檢查器",
    slideBackground: "投影片背景",
    slide: "投影片",
    deckStyle: "簡報樣式",
    designSystemTweaks: "設計系統微調",
    closeTweaks: "關閉微調",
    controls: "Controls",
    noDeckStyleControls: "尚無簡報樣式控制項。",
    type: "Type",
    color: "Color",
    size: "Size",
    line: "Line",
    regular: "Reg",
    medium: "Med",
    semi: "Semi",
    bold: "Bold",
    left: "Left",
    center: "Center",
    right: "Right",
    justify: "Justify",
    distribute: "分佈",
    fill: "Fill",
    tint: "Tint",
    transparent: "Transparent",
    alpha: "Alpha",
    stroke: "Stroke",
    radius: "Rad",
    strokeColor: "描邊色彩",
    spacing: "Spacing",
    x: "X",
    y: "Y",
    position: "位置",
    horizontal: "水平",
    vertical: "垂直",
    top: "頂端",
    middle: "置中",
    bottom: "底部",
    rotation: "旋轉",
    layoutDimensions: "版面與尺寸",
    width: "寬度",
    height: "高度",
    appearance: "外觀",
    opacity: "不透明度",
    cornerRadius: "圓角半徑",
    strokeWeight: "描邊粗細",
    typography: "排版",
    weight: "字重",
    italic: "斜體",
    underline: "底線",
    bulletList: "項目符號清單",
    numberedList: "編號清單",
    align: "對齊",
    decreaseSize: "縮小字型",
    increaseSize: "放大字型",
    mixed: "混合",
    textColor: "文字顏色",
    primary: "Primary",
    secondary: "Secondary",
    accent: "Accent",
    surface: "Surface",
    background: "Background",
    text: "Text",
    muted: "Muted",
    white: "White",
    black: "Black",
    slate: "Slate",
    blue: "Blue",
    cyan: "Cyan",
    emerald: "Emerald",
    amber: "Amber",
    rose: "Rose",
    arrange: "排列",
    order: "順序",
    bringToFront: "移到最上層",
    sendToBack: "移到最下層",
    copyStyle: "複製樣式",
    pasteStyle: "貼上樣式",
    copy: "複製",
    paste: "貼上",
    insertRowAbove: "在上方插入列",
    insertRowBelow: "在下方插入列",
    deleteRow: "刪除列",
    insertColumnLeft: "在左側插入欄",
    insertColumnRight: "在右側插入欄",
    deleteColumn: "刪除欄",
  },
  animations: {
    appear: "出現",
    fade: "淡入",
    slideUp: "向上滑入",
    zoom: "縮放",
    elementFallback: "元素 {{index}}",
    title: "轉場",
    close: "關閉轉場面板",
    emptyTitle: "還沒有動畫。",
    emptyDescription: "選取投影片上的物件以新增轉場。",
    selectObject: "選取要製作動畫的物件",
    addTransition: "新增轉場",
    effect: "效果",
    trigger: "觸發方式",
    onClick: "按一下",
    byParagraph: "依段落",
    expand: "展開轉場",
    collapse: "收合轉場",
    reorder: "重新排序轉場",
    remove: "移除轉場",
    play: "播放",
    clearAll: "全部清除",
    elements: "元素",
    autoFill: "自動填充",
    noAnimatableElements: "此幻燈片未偵測到可動畫元素。",
  },
  generateSlides: {
    title: "用 AI 產生幻燈片",
    description: "描述你的簡報主題，AI 會為你產生投影片。",
    topic: "主題",
    topicPlaceholder: "例如：React Hooks 入門、Q4 銷售報告、2025 產品路線圖...",
    slides: "幻燈片",
    slideCount_one: "{{count}} 張幻燈片",
    slideCount_other: "{{count}} 張幻燈片",
    styleOptional: "風格（可選）",
    stylePlaceholder: "例如：極簡、企業風...",
    generateImages: "產生圖片",
    generateImagesDescription: "AI 將使用 Gemini 為視覺幻燈片產生圖片",
    referenceImages: "參考圖片（可選，用於品牌一致性）",
    referenceImagesDescription: "上傳圖片以匹配產生圖片的視覺風格",
    generating: "正在生成幻燈片...",
    generate: "生成幻燈片",
  },
  history: {
    unknownTime: "未知時間",
    justNow: "剛剛",
    minutesAgo: "{{count}} 分鐘前",
    hoursAgo: "{{count}} 小時前",
    daysAgo: "{{count}} 天前",
    slideCount_one: "{{count}} 張幻燈片",
    slideCount_other: "{{count}} 張幻燈片",
    versionRestored: "版本已恢復",
    versionRestoredDescription: "幻燈片已回滾到所選快照。",
    restoreFailed: "恢復失敗",
    restoreFailedDescription: "無法恢復此幻燈片版本。",
    backToSavedVersions: "返回已儲存版本",
    savedVersions: "已儲存版本",
    description: "瀏覽已儲存的幻燈片版本並恢復以前的快照。",
    untitled: "未命名",
    snapshotUnavailable: "快照不可用",
    slideNumber: "幻燈片 {{number}}",
    noSlidesInSnapshot: "此快照中沒有幻燈片。",
    restoreThisVersion: "恢復此版本",
    noSavedVersions: "還沒有已儲存版本",
    noSavedVersionsDescription: "以後編輯幻燈片前會自動儲存版本。",
  },
  editorSidebar: {
    selectSlide: "選取幻燈片 {{number}}",
    duplicateSlide: "複製幻燈片",
    duplicate: "複製",
    deleteSlide: "刪除幻燈片",
    slideDeleted: "投影片已刪除",
    delete: "刪除",
    generatingSlide: "正在生成幻燈片",
    uploadFailed: "上傳失敗",
    uploadAttachedFileFailed: "無法上傳附加檔案。",
    newSlideSaveFailed: "無法儲存新投影片,請重試。",
    addSlides: "新增幻燈片",
    addEmptySlide: "新增空白幻燈片",
    noAi: "無 AI",
    duplicateCurrentSlide: "複製目前幻燈片",
    promptPlaceholder: "描述你想要的幻燈片...",
    newSlide: "新增投影片",
    closeAddSlides: "關閉",
    describeThisSlide: "描述這張投影片",
    cut: "剪下",
    copy: "複製",
    paste: "貼上",
    skipSlide: "略過投影片",
    unskipSlide: "取消略過投影片",
  },
  presentation: {
    loadFailed: "無法載入此簡報。",
    tryAgain: "重試",
    presenterView: "簡報者檢視",
    speakerNotes: "簡報者備忘稿",
    noNotesForSlide: "此投影片沒有備忘稿",
    upNext: "下一張",
    endOfDeck: "簡報結束",
    closePresenterView: "關閉簡報者檢視",
    noSlides: "沒有可播放的幻燈片",
    previousSlide: "上一張幻燈片",
    nextSlide: "下一張幻燈片",
    exitPresentation: "結束簡報",
    enterFullscreen: "進入全螢幕",
    clickToEnterFullscreen: "點選進入全螢幕",
  },
  deckEditor: {
    lookingForDeck: "正在尋找此幻燈片",
    joinTeamToOpen: "加入團隊以開啟此幻燈片",
    deckUnavailable: "幻燈片不可用",
    checkingSharedAccess: "正在檢查此簡報是否與你的帳戶共用。",
    joinTeamDescription:
      "此連結指向團隊簡報。加入上方顯示的團隊後，幻燈片會自動在此開啟。",
    deckUnavailableDescription: "此幻燈片可能已被移除，或你的帳戶無權存取。",
    privateDeckTitle: "此幻燈片是私密的",
    privateDeckDescription:
      "請求擁有者授予存取權限以開啟此幻燈片。傳送請求後，擁有者會收到通知。",
    signedInAs: "你已登入為",
    accessRequestSentDescription:
      "擁有者已收到通知。取得存取權限後，你就可以開啟此幻燈片。",
    accessRequestRecordedDescription:
      "你的請求已記錄。取得存取權限後，你就可以開啟此幻燈片。",
    requestAccessPending: "正在傳送請求…",
    accessRequestSent: "請求已傳送",
    requestAccess: "請求存取權限",
    signInToRequestAccess: "登入以請求存取權限",
    requestAccessDialogTitle: "請求存取權限",
    requestAccessDialogDescription:
      "請登入，或輸入擁有者應與此簡報分享的電子郵件地址。",
    requestAccessSignIn: "登入或註冊",
    requestAccessOr: "或",
    requestAccessEmailLabel: "電子郵件地址",
    requestAccessEmailPlaceholder: "you@example.com",
    requestAccessEmailHint: "取得存取權限後，請使用此電子郵件登入以查看簡報。",
    requestAccessWithEmail: "透過電子郵件請求",
    requestAccessEmailRequired: "請輸入有效的電子郵件地址。",
    requestingAccess: "正在請求存取權限…",
    accessRequestSentWithEmail: "已請求擁有者與 {{email}} 分享此簡報。",
    accessRequestFailed: "無法請求存取權限。請再試一次。",
    accessApprovalTitle: "已授予存取權限",
    accessApprovalAlreadyTitle: "已授予存取權限",
    accessApprovalMessage: "{{email}} 現在可以開啟此簡報。",
    accessApprovalAlreadyMessage: "{{email}} 已經可以存取此簡報。",
    accessApprovalErrorTitle: "無法授予存取權限",
    accessApprovalInvalid: "此存取請求無效或已過期。",
    accessApprovalSignInTitle: "登入以允許存取",
    accessApprovalSignInMessage: "請以簡報擁有者或管理員身分登入以核准此請求。",
    accessApprovalOpenDeck: "開啟簡報",
    accessApprovalSignIn: "登入",
    accessApprovalLoading: "正在授予存取權限...",
    backToDecks: "返回幻燈片",
    tryAgain: "重試",
    imageUploadFailed: "圖片上傳失敗",
    imageUploadNeedsBuilder:
      "請從代理編寫器的模型選單連接 Builder.io，以便將圖片上傳到投影片。即使沒有提供者，將圖片拖到空白畫布上仍可傳送給代理。",
    imageAdded: "圖片已新增",
    imageUploadError: "上傳此圖片時出了點問題。",
    exportFailed: "匯出失敗",
    deckHasNoSlides: "幻燈片沒有頁面。",
    pdfRenderFailed: "無法渲染 PDF。",
    buildingDeck: "正在建置幻燈片",
    layoutOverflowWarning: "版面溢出",
    layoutOverflowDetails: "顯示溢出詳細資訊",
    layoutOverflowVertical: "垂直溢出：{{pixels}}px",
    layoutOverflowHorizontal: "水平溢出：{{pixels}}px",
    dismissLayoutWarning: "關閉版面警告",
    slidesAdded_one: "已新增 {{count}} 張幻燈片",
    slidesAdded_other: "已新增 {{count}} 張幻燈片",
    unsavedChangesTitle: "未儲存的變更",
    unsavedChangesDescription:
      "如果現在離開或重新載入，可能會遺失尚未儲存的變更。確定要退出嗎？",
    keepEditing: "繼續編輯",
    leaveWithoutSaving: "不儲存並退出",
  },
  designSystemSetup: {
    importedBrand: "匯入的品牌",
    figFileRequired:
      "請選取 .fig 檔案（在 Figma 中：File > Save local copy）。",
    figFileTooLarge: "檔案太大（最大 {{maxSize}}）。",
    figParseFailed: "無法為該 Figma 檔案啟動 Builder 索引。",
    updated: "設計系統已更新",
    sourceLabel: "來源",
    sourceFigma: "Figma 檔案",
    sourceCode: "程式碼檔案",
    sourceGitHub: "GitHub 儲存庫",
    sourceMixed: "多個來源",
    sourceBuilder: "Builder 設計系統智慧",
    sourceIndexingDescription:
      "Builder 仍在處理此來源。結果準備好後會顯示於此。",
    sourceIndexedDescription: "已索引 {{docs}} 份文件 · {{tokens}} 個權杖",
    sourceIndexedDocsOnly: "{{docs}} 份文件已索引",
    sourceIndexedTokensOnly: "{{tokens}} 個權杖已索引",
    sourceUnavailableDescription: "Builder 無法載入最新的來源結果。",
    sourceOpenInBuilder: "在 Builder 中開啟",
    syncSource: "同步來源",
    syncingSource: "正在同步來源...",
    syncStarted: "設計系統同步已開始",
    syncFailed: "無法同步設計系統",
    loadFailed: "無法載入此設計系統。請關閉後重試。",
    otherSources: "其他",
    otherSourcesDescription: "從網站、程式碼、檔案或現有設計系統連接。",
    chooseSourcePrompt: "選擇一個來源進行設定",
    loading: "正在載入設計系統",
    updateFailed: "更新失敗",
    generationStarted: "設計系統生成已開始",
    generationStartedDescription: "你可以繼續工作，代理會處理這些來源。",
    editTitle: "編輯設計系統",
    newTitle: "新建設計系統",
    editDescription: "更新你的品牌識別。",
    newDescription:
      "透過 Builder DSI 連接 Figma、程式碼和選用的 design.md 指引。",
    companyBrand: "公司 / 品牌",
    companyBrandPlaceholder: "Acme Inc. — 我們建置開發者工具...",
    figmaFile: "連接 Figma",
    parsingFigmaFile: "正在啟動 Builder DSI 索引...",
    uploadFigDescription: "上傳本機 .fig 副本以用於 Builder DSI",
    builderIndexingStarted: "Builder 索引已啟動",
    builderIndexingDescription:
      "Builder is indexing {{title}} into a reusable design system.",
    builderDesignSystemId: "設計系統",
    builderJobId: "Job",
    openInBuilder: "在 Builder 中開啟",
    decodeFailed: "解碼失敗：{{error}}",
    websiteUrl: "網站 URL",
    websitePlaceholder: "example.com 或 Nike",
    add: "新增",
    githubRepository: "連接程式碼：GitHub 倉庫",
    githubRef: "分支、標籤或提交（選用）",
    githubPaths: "檔案或資料夾，以逗號分隔（選用）",
    codeFiles: "連接程式碼檔案",
    codeFilesDrop: "CSS、Tailwind 設定、主題檔案、design.md — 拖放或點選",
    documents: "檔案和簡報",
    documentsDrop: "PPTX、DOCX、PDF、Markdown、TXT — 品牌指南、設計檔案",
    visualReferences: "截圖和視覺參考",
    visualReferencesDrop: "產品截圖、情緒板、徽標",
    forkExisting: "派生現有設計系統",
    brandNotes: "品牌備註",
    additionalNotes: "其他備註",
    notesPlaceholder: "設計偏好、限制、品牌指南...",
    customInstructions: "自訂說明",
    customInstructionsPlaceholder:
      "例如：始終用單一資料標題頁面開場。不要使用超過 3 項的專案符號清單。幻燈片標題少於 6 個詞...",
    customInstructionsDescription:
      "隨設計系統儲存。代理每次使用它生成幻燈片時都會重新應用。",
    cancel: "取消",
    saving: "正在儲存...",
    saveChanges: "儲存更改",
    continueToGeneration: "繼續生成",
    removeItem: "移除 {{item}}",
    chooseAnotherFile: "選取其他檔案",
  },
  home: {
    loadFailed: "無法載入內容",
    loadFailedDescription: "您儲存的內容仍然可用。請檢查連線並重試。",
    retry: "重試",
    decksTitle: "幻燈片",
    newDeck: "新建幻燈片",
    deckLengthQuestion: "這份幻燈片需要多長？",
    deckLengthHeader: "幻燈片長度",
    deckLengthShort: "短（3–5 張）",
    deckLengthMedium: "中（6–10 張）",
    deckLengthLong: "長（11+ 張）",
    deckLengthSingleVisual: "只要一個視覺頁面",
    generationStartFailed: "無法開始生成幻燈片",
    generationStartFailedDescription:
      "新幻燈片尚未儲存完成，因此未讓代理針對缺失的幻燈片開始工作。你的提示已儲存，可以重試。",
    showAllDecks: "顯示所有幻燈片",
    all: "全部",
    showMineDecks: "顯示我建立的幻燈片",
    mine: "我的",
    createDeckOrVisual: "建立簡報",
    noMineDecks: "你還沒有建立任何幻燈片。",
    noDecksMatchSearch: "沒有符合搜尋條件的簡報。",
    deleteDeckTitle: "刪除幻燈片？",
    deleteDeckDescription: "這會永久刪除此幻燈片及其所有頁面。此操作無法撤銷。",
    cancel: "取消",
    delete: "刪除",
    newDeckPromptTitle: "新建簡報",
    newDeckPlaceholder: "描述你想產生的簡報...",
    skipPrompt: "跳過提示",
    firstDeckPromptTitle: "我們要產生什麼類型的簡報？",
    firstDeckSkip: "跳過",
    chooseReferences: "選擇參考資料",
    addDesignSystem: "+ 設計系統",
    importFrom: "匯入來源",
    attachedFiles: "已附加檔案",
    imported: "已匯入",
    importedReferenceDeck: "已匯入的參考投影片",
    referenceImportSuccess: "匯入成功",
    referenceImportSelected: "「{{title}}」現已設為參考投影片。",
    continue: "繼續",
    continueToGenerate: "繼續生成",
    designSystem: "設計系統",
    referenceDeck: "參考幻燈片",
    referenceDeckPlaceholder: "參照現有幻燈片的風格",
    referenceDeckNone: "不使用參考",
    referenceDeckStarredGroup: "已加星號",
    referenceDeckOtherGroup: "其他幻燈片",
    referenceReviewTitle: "檢查參考內容",
    referenceReviewDescription:
      "除非移除，否則保留預設設計系統和參考投影片。搜尋其他投影片、匯入檔案，或準備好後繼續。",
    chooseAnotherDeck: "選擇其他投影片",
    noMatchingDecks: "找不到相符的投影片。",
    googleSlidesImportLabel: "Slides",
    googleSlidesReferenceTitle: "Google Slides 投影片",
    googleSlidesReferenceConnect: "連接 Google Drive 以匯入 Slides 投影片。",
    googleSlidesReferenceChoose: "選擇 Google Slides 投影片",
    googleSlidesReferenceUrl: "貼上 Google Slides 連結",
    googleSlidesReferencePicking: "處理中...",
    googleSlidesReferenceConnected:
      "已連接。選擇 Google Slides 投影片，將其匯入為可重複使用的參考內容。",
    googleSlidesReferenceUnavailable:
      "連接 Google Drive 並設定 Picker 後，才能匯入 Google Slides 投影片。",
    skipReferences: "跳過參考內容",
    workspaceDefaultBadge: "工作區預設",
    setWorkspaceDefault: "設為工作區預設",
    clearWorkspaceDefault: "清除工作區預設",
    workspaceDefaultSet: "已設為工作區預設",
    workspaceDefaultCleared: "已清除工作區預設",
    duplicateFailed: "無法複製該簡報",
    workspaceDefaultFailed: "無法更新工作區預設設定",
    workspaceDefaultConfirmTitle: "設為工作區預設？",
    workspaceDefaultDeckShareBody:
      "“{{title}}”目前為私密。將其設為工作區預設後，它會與此工作區的所有成員共用，新建的幻燈片將以它作為樣式參考。成員在新建幻燈片時可以選擇其他參考幻燈片。",
    workspaceDefaultSystemShareBody:
      "“{{title}}”目前為私密。將其設為工作區預設後，它會與此工作區的所有成員共用，新建的幻燈片將使用它。已設定個人預設設計系統的成員將保留其個人預設設定。",
    workspaceDefaultConfirmAction: "設為預設",
    starDeck: "加星號",
    unstarDeck: "取消星號",
    none: "無",
    defaultSuffix: "（預設）",
    signInTitle: "登入以建立幻燈片",
    signInDescription:
      "生成幻燈片前需要登入。我們已儲存你的提示；回來後就可以繼續。",
    signInDescriptionWithFiles:
      "生成幻燈片前需要登入。我們已儲存你的提示；回來後請重新附加檔案。",
    signIn: "登入",
    emptyTitle: "還沒有簡報",
    createFirstDeck: "建立你的第一份簡報",
    emptyDescription: "使用 AI 產生精美簡報。",
  },
};

export default messages;
