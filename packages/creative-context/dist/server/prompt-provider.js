...
export function publishedBrandContextInput(profileId, dnaVersionId, payload) {
    ...
    return {
        ...
        numericScales: payload.numericScales ?? visual.numericScales ?? visual.scales,
        voiceDescriptors: payload.voiceDescriptors ?? voice.descriptors ?? voice.tone,
        layoutPatterns: payload.layoutPatterns ?? visual.layoutPatterns ?? payload.motifs,
...
export function registerCreativeContextPromptProvider() {
    ...
    const unregister = registerPromptContextProvider({
        ...
        async load() {
            ...
            if (!profile || !dna || dna.status !== "published")
                return null;
            return {
                label: "Published brand context",
                content: compilePublishedBrandContext(publishedBrandContextInput(profile.id, dna.id, dna.payload)),
