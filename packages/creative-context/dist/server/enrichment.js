...
import { appendMediaEnrichmentVersion, getActiveEmbeddingSet, getCreativeContextItem, recordEmbeddingMetadata, } from "../store/index.js";
import { getCreativeContext } from "./context.js";
import { readCreativeContextMedia } from "./media.js";
export async function projectCreativeContextMedia(mediaId) {
    const { getDb, schema, projections } = getCreativeContext();
    if (!projections?.media)
        return { mediaId, projected: false };
...
export async function enrichCreativeContextMedia(input) {
    ...
    if (loaded.mimeType !== "image/png" &&
        ...
        throw new Error("Creative context media enrichment supports PNG, JPEG, WebP, or GIF images.");
    }
    const { vectorAdapter, enrichment } = getCreativeContext();
    const fingerprint = fingerprintMedia(loaded.data, loaded.mimeType);
    const palette = await extractDominantColors(loaded.data, input.paletteLimit ?? 6).catch(() => []);
    ...
    const imageMimeSupported = !family?.supportedImageMimeTypes ||
    ...
    return {
        ...
        embeddingSkippedReason: set && family && !imageMimeSupported
