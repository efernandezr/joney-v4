...
import { fetchRemoteArtifact, parsePrivateBlobHandle, readPrivateArtifact, } from "../connectors/private-artifacts.js";
...
import { matchesCreativeSearchMode, shouldUsePostgresFts, } from "../search/mode.js";
import { queryPostgresFts } from "../search/postgres-fts.js";
import { createContextPack, createJob, getActiveEmbeddingSet, getCreativeContextItem, getContextSource, listAccessibleLexicalCandidates, listAccessibleSearchDocuments, listEmbeddingMetadata, } from "../store/index.js";
import { delimitUntrustedReference, UNTRUSTED_REFERENCE_ROLE, } from "../untrusted-reference.js";
...
function ranked(document, score, reason) {
    return { key: document.chunkId, value: document, score, reason };
}
function rankQuality(document) {
    const curation = document.starred || document.curationRank === "canonical"
        ? 1
        : document.curationRank === "exemplar"
            ? 0.5
            : 0;
    const ageDays = Math.max(0, (Date.now() - new Date(document.updatedAt).getTime()) / 86_400_000);
    const recency = Number.isFinite(ageDays) ? 0.03 / (1 + ageDays / 30) : 0;
    const priorReuse = Math.min(0.04, Math.log1p(document.priorReuseCount) * 0.01);
    const helpful = Math.min(0.04, Math.log1p(document.helpfulFeedbackCount) * 0.015);
    return curation + recency + priorReuse + helpful;
}
async function queryImage(input) {
    if (!input.imageBlobRef && !input.mediaId)
        return null;
    if (!isPostgres())
    ...
    if (input.mediaId) {
        ...
        const row = rows[0];
        if (!row)
            throw new Error("Visual-query media was not found");
        ...
        if (!media)
            throw new Error("Visual-query media is not accessible");
    ...
    if (reference) {
        ...
        if (!handle)
            throw new Error("Visual query requires a valid private image blob reference");
...
export async function performCreativeContextSearch(input) {
    ...
    const lexicalDocuments = lexicalItemIds.length
        ...
        : [];
    let documents = [
        ...new Map([...baseDocuments, ...lexicalDocuments].map((document) => [
            document.chunkId,
            document,
        ])).values(),
    ];
    let byChunk = new Map(documents.map((document) => [document.chunkId, document]));
    const lanes = {};
    if (query) {
        ...
        if (isPostgres() && shouldUsePostgresFts(input.matchMode)) {
            ...
            const ftsDocuments = hits.length
                ...
                : [];
            documents = [
                ...new Map([...documents, ...ftsDocuments].map((document) => [
                    document.chunkId,
                    document,
                ])).values(),
            ];
            byChunk = new Map(documents.map((document) => [document.chunkId, document]));
            lanes.fts = hits.flatMap((hit) => {
                const document = byChunk.get(hit.chunkId);
                return document &&
                    matchesCreativeSearchMode(`${document.title}\n${document.summary ?? ""}\n${document.body}`, query, input.matchMode ?? "allTerms")
    ...
    if (isPostgres()) {
        ...
        const family = activeSet
            ? (families.find((candidate) => candidate.id === activeSet.family &&
                candidate.model === activeSet.model &&
                candidate.version === activeSet.version) ?? null)
            : null;
        if (family) {
            const set = activeSet;
            ...
            if (set && vectorAdapter && set.dimensions === family.dimensions) {
                if (image &&
                    ...
                    throw new Error(`The active embedding family does not support ${image.mimeType} visual queries. Use PNG or JPEG.`);
                }
                ...
                if (metadata.length) {
                    ...
                    const metadataByVector = new Map(metadata.map((entry) => [entry.vectorKey, entry]));
                    ...
                    const vectorDocuments = vectorItemIds.length
                        ...
                        : [];
                    documents = [
                        ...new Map([...documents, ...vectorDocuments].map((document) => [
                            document.chunkId,
                            document,
                        ])).values(),
                    ];
                    ...
                    lanes.vector = matches.flatMap((match) => {
                        ...
                        return document
                            ? [
                                ranked(document, match.score, image ? "visual similarity" : "semantic similarity"),
                            ]
                            : [];
                    });
    ...
    if (image && !vectorAvailable) {
        throw new Error("Visual search needs a configured multimodal embedding provider and indexed visual context.");
    }
    const nonemptyLanes = Object.fromEntries(Object.entries(lanes).filter(([, candidates]) => candidates && candidates.length));
    ...
    const collapsed = fused.filter((candidate) => {
        if (seenItems.has(candidate.value.itemId))
            return false;
    ...
    const results = await Promise.all(page.map(async (candidate) => ({
        ...candidate.value,
        body: undefined,
        summary: undefined,
    ...
    if (input.snapshot !== false &&
        ...
        const current = (await readAppState("creative-context").catch(() => null));
        if (current?.contextMode !== "off") {
            await writeAppState("creative-context", {
                contextMode: "auto",
                currentPackId: pack.id,
                pinnedPackId: typeof current?.pinnedPackId === "string"
    ...
    return {
        query: query || null,
        results,
        nextCursor: nextCursor(offset, input.limit, offset + input.limit < collapsed.length),
        contextPackId,
        coverage: {
            mode: Object.keys(nonemptyLanes).length > 1
                ? "fused"
...
export async function ensureContextItemHydration(itemId) {
    const detail = await getCreativeContextItem(itemId);
    if (!detail || detail.version.parseStatus !== "pending")
        return null;
    ...
    const rows = await getDb()
        ...
        .where(and(eq(schema.contextJobs.sourceId, source.id), inArray(schema.contextJobs.status, ["queued", "running", "paused"]), eq(schema.contextJobs.ownerEmail, source.ownerEmail)));
    const existing = rows.find((row) => {
        try {
            const request = JSON.parse(row.request);
            return (Array.isArray(request.itemExternalIds) &&
