import { getH3App, getSession, runWithRequestContext, } from "@agent-native/core/server";
import { eq } from "drizzle-orm";
import { parsePrivateBlobHandle, readPrivateArtifact, } from "../connectors/private-artifacts.js";
import { CREATIVE_CONTEXT_MEDIA_ROUTE } from "../media-url.js";
import { getCreativeContextItem, readPendingCreativeContextMedia, } from "../store/index.js";
...
export function createCreativeContextMediaPlugin() {
    return async (nitroApp) => {
        getH3App(nitroApp).use(CREATIVE_CONTEXT_MEDIA_ROUTE, async (event) => {
            ...
            const url = new URL(event.req.url, event.req.headers?.get?.("origin") ?? "http://localhost");
            const mediaId = url.searchParams.get("mediaId");
            let loaded;
            try {
                loaded = await runWithRequestContext({ userEmail, orgId }, () => readCreativeContextMedia({
...
export async function readCreativeContextMedia(input) {
    ...
    const media = detail && input.mediaId
    ...
    if (!handle)
        throw new Error("Creative context media has no private blob");
    return {
        data: await readPrivateArtifact(handle, getCreativeContext().connectorContext),
