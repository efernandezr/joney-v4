...
const readPayloadSchema = z
    ...
    .strict();
export const creativeContextA2ARequestSchema = z.discriminatedUnion("operation", [
    z
        .object({
...
const responseEnvelopeSchema = z
    ...
    .strict();
export function hasIsolatedCreativeContextA2A() {
    return Boolean(process.env.CREATIVE_CONTEXT_A2A_URL?.trim());
}
export function decodeCreativeContextA2ARequest(requestToken) {
    ...
    return creativeContextA2ARequestSchema.parse(JSON.parse(Buffer.from(requestToken, "base64url").toString("utf8")));
}
export function createCreativeContextA2AResponseToken(request, result) {
    const response = {
        protocol: PROTOCOL,
...
export async function callIsolatedCreativeContextA2A(operation, payload, options = {}) {
    const url = parseConfiguredUrl();
    const userEmail = getRequestUserEmail()?.trim().toLowerCase();
    if (!userEmail) {
        throw new Error("Isolated Creative Context requires an authenticated caller identity");
    }
    const orgSecret = process.env.CREATIVE_CONTEXT_A2A_KEY?.trim();
    if (!orgSecret && !process.env.A2A_SECRET?.trim()) {
        throw new Error("CREATIVE_CONTEXT_A2A_URL requires CREATIVE_CONTEXT_A2A_KEY or A2A_SECRET");
    }
    ...
    const requestToken = Buffer.from(JSON.stringify(request), "utf8").toString("base64url");
    const prompt = [
    ...
    const callAgent = options.callAgent ?? (await import("@agent-native/core/a2a")).callAgent;
    let responseText;
    try {
    ...
    if (responseText.length > MAX_TOKEN_LENGTH + 1_000) {
        throw new Error("Isolated Creative Context A2A response exceeds the protocol limit");
    }
    const tokenPattern = new RegExp(`${RESPONSE_PREFIX}:${request.requestId}:([A-Za-z0-9_-]+)`);
    const encoded = tokenPattern.exec(responseText)?.[1];
    if (!encoded) {
        throw new Error("Isolated Creative Context returned a malformed protocol response");
    }
    let envelope;
    try {
        envelope = responseEnvelopeSchema.parse(JSON.parse(Buffer.from(encoded, "base64url").toString("utf8")));
    }
    catch {
        throw new Error("Isolated Creative Context returned a malformed protocol response");
    }
    if (envelope.requestId !== request.requestId ||
        ...
        throw new Error("Isolated Creative Context response did not match the request");
    }
    return normalizeRemoteResult(operation, envelope.result);
}
function normalizeRemoteResult(operation, result) {
    switch (operation) {
        case "resolve":
        case "validate": {
            const value = result;
...
function canonicalGenerationRecord(result) {
    const value = result;
    return {
        id: value?.id,
        appId: value?.appId,
...
function parseConfiguredUrl() {
    ...
    if (!value)
        throw new Error("CREATIVE_CONTEXT_A2A_URL is not configured");
    let url;
    try {
        url = new URL(value);
    }
...
export function isolatedResolvePayload(input) {
    return {
        query: input.query,
        role: input.role,
        limit: input.limit,
...
//# sourceMappingURL=isolated-a2a.js.map