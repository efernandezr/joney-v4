...
export function googleSlidesPickerSelections(value) {
...
async function withGooglePickerTimeout(promise, message, timeoutMs = GOOGLE_PICKER_LOAD_TIMEOUT_MS) {
    let timeout;
    try {
...
async function loadGooglePicker() {
    if (!window.gapi) {
        window.__creativeContextGooglePickerScript ??= new Promise((resolve, reject) => {
        ...
        await withGooglePickerTimeout(window.__creativeContextGooglePickerScript, "Google Picker script timed out.");
    }
    if (!window.gapi?.load)
        throw new Error("Google Picker did not initialize.");
    await withGooglePickerTimeout(new Promise((resolve, reject) => {
...
export async function chooseGoogleSlidesPresentations(input) {
    ...
    return withGooglePickerTimeout(new Promise((resolve, reject) => {
    ...
    }), "Google Picker selection timed out.", GOOGLE_PICKER_SELECTION_TIMEOUT_MS);
}
//# sourceMappingURL=google-slides-picker.js.map