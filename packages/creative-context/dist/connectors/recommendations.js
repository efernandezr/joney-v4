import { asRecord, connectorConnectionId, executeConnectorProviderRequest, positiveLimit, stringValue, } from "./provider-response.js";
export async function recommendContextRoots(input, context) {
    ...
    if (input.provider === "notion") {
        const connectionId = await connectorConnectionId("notion", config, context.resolveConnection);
        ...
        const recommendations = (Array.isArray(payload?.results) ? payload.results : []).flatMap((value) => {
    ...
    if (input.provider === "google-slides") {
        const connectionId = await connectorConnectionId("google_drive", config, context.resolveConnection);
        const payload = asRecord(await executeConnectorProviderRequest(context.providerApi, {
            ...
            query: {
                ...
                fields: "files(id,name,modifiedTime,webViewLink,parents,driveId,shared,ownedByMe)",
            },
            connectionId,
        }));
        const recommendations = (Array.isArray(payload?.files) ? payload.files : []).flatMap((value) => {
    ...
    const connectionId = await connectorConnectionId("figma", config, context.resolveConnection);
    const projectIds = new Set();
    if (input.figmaProjectId?.trim())
        projectIds.add(input.figmaProjectId.trim());
    if (input.figmaTeamId?.trim()) {
        const projects = asRecord(await executeConnectorProviderRequest(context.providerApi, {
            provider: "figma",
            method: "GET",
    ...
    if (projectIds.size === 0) {
        return {
            ...
            unavailableReason: "Figma has no global recent-files API. Choose a team or project before requesting file recommendations.",
        };
    }
    ...
    recommendations.sort((left, right) => timestamp(right.sourceModifiedAt) - timestamp(left.sourceModifiedAt));
    return recommendationResult(recommendations.slice(0, limit));
}
function recommendationResult(recommendations) {
    return {
        recommendations,
        persisted: false,
...
function notionRecommendationTitle(page) {
