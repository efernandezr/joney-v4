...
import { fetchRemoteArtifact, parsePrivateBlobHandle, readPrivateArtifact, sanitizeProvenanceUrl, sanitizeRemoteArtifact, serializePrivateBlobHandle, storePrivateArtifact, } from "./private-artifacts.js";
import { asRecord, cursorOffset, positiveLimit, stringValue, } from "./provider-response.js";
export class UploadContextConnector {
    kind = "upload";
    label = "Uploaded files";
    supportsIncremental = false;
    async inventory(request, _context) {
    ...
    async fetch(request, context) {
        ...
        if (loadedMetadata.standaloneImage === true) {
            if (!storageKey) {
                throw new Error(`Uploaded image ${entry.id} must be persisted in private blob storage.`);
            }
            ...
            if (!mimeType || !contentHash) {
                throw new Error(`Uploaded image ${entry.id} is missing media metadata.`);
            }
            const palette = Array.isArray(loadedMetadata.palette)
                ? loadedMetadata.palette.filter((value) => typeof value === "string")
                : [];
        ...
        const normalized = await Promise.all(parsed.parts.map(async (part) => {
            const imageMedia = await Promise.all((part.images ?? []).map(async (image) => {
                const safeMimeType = resolveSupportedImageMimeType(image.mimeType, image.name);
                if (!safeMimeType) {
                    throw new Error(`Embedded image ${image.name} uses an unsupported media type.`);
                }
                        ...
                        ? [
                            {
                                kind: "document",
                                ...(entry.mimeType ? { mimeType: entry.mimeType } : {}),
                                accessMode: storageKey || entry.storageKey
                                    ? "private"
                                    : "public",
        ...
        const parent = normalizeContextItem({
            ...
            media: canonicalUrl || storageKey || entry.storageKey
                ? [
                    {
                        kind: "document",
                        ...(entry.mimeType ? { mimeType: entry.mimeType } : {}),
                        accessMode: storageKey || entry.storageKey ? "private" : "public",
                        ...(storageKey || entry.storageKey
                            ? { storageKey: storageKey ?? entry.storageKey }
                            : { url: canonicalUrl }),
                        ...(canonicalUrl ? { provenanceUrl: canonicalUrl } : {}),
...
function parseUploadEntries(config) {
    ...
    return config.items.map((value, index) => {
        ...
        if (stringValue(item?.data) || stringValue(item?.base64)) {
            throw new Error(`Upload ${id} contains inline file data; persist it in blob storage and pass storageKey or url.`);
        }
        ...
        if (!storageKey && !blobHandle && !url && !extractedText) {
            throw new Error(`Upload ${id} requires storageKey, url, or pre-extracted text.`);
        }
        return {
            id,
            title: stringValue(item?.title) ??
                ...
                `Upload ${index + 1}`,
            fileName: stringValue(item?.fileName) ?? stringValue(item?.title) ?? `${id}.bin`,
            storageKey,
            blobHandle,
            url,
...
async function loadAndParseUpload(entry, context) {
    ...
    if (data) {
        const imageMimeType = resolveSupportedImageMimeType(mimeType, entry.fileName);
        if (imageMimeType) {
...
function resolveSupportedImageMimeType(mimeType, fileName) {
...
function isSupportedImageUpload(mimeType, fileName) {
    return resolveSupportedImageMimeType(mimeType, fileName) !== null;
}
function assertSafeImageBytes(data, mimeType) {
...
function asParsedUpload(loaded, entry) {
    const structured = loaded;
...
function findUploadEntry(config, inventoryItem) {
    ...
    const entry = byIndex?.id === inventoryItem.externalId
        ? byIndex
        : entries.find((candidate) => candidate.id === inventoryItem.externalId);
    if (!entry)
        throw new Error(`Upload ${inventoryItem.externalId} not found.`);
    return entry;
}
import { extractDominantColors, fingerprintMedia, } from "@agent-native/core/ingestion";
