...
import { fetchRemoteArtifact, rehostRemoteMedia, storePrivateArtifact, } from "./private-artifacts.js";
import { asRecord, connectorConnectionId, cursorOffset, executeConnectorProviderRequest, isContextConnectorQuotaError, positiveLimit, stringArray, stringValue, } from "./provider-response.js";
const SLIDES_MIME_TYPE = "application/vnd.google-apps.presentation";
export const GOOGLE_SLIDES_CONTEXT_OAUTH_SCOPES = [
    "https://www.googleapis.com/auth/drive.file",
];
export class GoogleSlidesContextConnector {
    kind = "google-slides";
    label = "Google Slides";
    ...
    verifiesContainerOwner(input) {
        ...
        const inventoryById = new Map(input.inventory.map((item) => [item.externalId, item]));
    ...
    async inventory(request, context) {
        const connectionId = await connectorConnectionId("google_drive", request.config, context.resolveConnection);
        ...
        if (presentationIds.length > 0) {
            ...
            const items = (await Promise.all(selected.map(async (presentationId) => {
                const file = asRecord(await executeConnectorProviderRequest(context.providerApi, {
                    provider: "google_drive",
                    method: "GET",
                    path: `/files/${encodeURIComponent(presentationId)}`,
                    query: {
                        fields: "id,name,mimeType,modifiedTime,webViewLink,size,parents,driveId,ownedByMe,shared,copyRequiresWriterPermission,capabilities(canCopy,canDownload,canShare),permissions(type,role,allowFileDiscovery,deleted,pendingOwner)",
                    },
                    connectionId,
            ...
            const nextCursor = nextOffset < presentationIds.length ? String(nextOffset) : null;
            return {
                items,
                nextCursor,
                complete: !nextCursor,
        ...
        const folderId = stringValue(request.config.folderId) ??
            googleDriveContainerIdFromUrl(request.config.folderUrl, "folder");
        const sharedDriveId = stringValue(request.config.sharedDriveId) ??
            googleDriveContainerIdFromUrl(request.config.sharedDriveUrl, "shared drive");
        if (!folderId && !sharedDriveId) {
            throw new Error("Google Slides connector config requires presentationIds, folderId, or sharedDriveId. Recent-file suggestions are not an import boundary until confirmed as presentationIds.");
        }
        const q = [
        ...
        const payload = asRecord(await executeConnectorProviderRequest(context.providerApi, {
            provider: "google_drive",
            method: "GET",
            path: "/files",
            query: {
                ...
                fields: "nextPageToken,files(id,name,mimeType,modifiedTime,webViewLink,size,parents,driveId,ownedByMe,shared,copyRequiresWriterPermission,capabilities(canCopy,canDownload,canShare),permissions(type,role,allowFileDiscovery,deleted,pendingOwner))",
    ...
    async fetch(request, context) {
        const connectionId = await connectorConnectionId("google_drive", request.config, context.resolveConnection);
        if (!connectionId) {
            throw new Error("Google Slides imports require a granted Google Drive workspace connection.");
        }
        const presentation = asRecord(await executeConnectorProviderRequest(context.providerApi, {
            provider: "google_slides",
            method: "GET",
            path: `/presentations/${encodeURIComponent(request.item.externalId)}`,
            query: {
                fields: "presentationId,title,locale,revisionId,pageSize,slides,masters,layouts,notesMaster",
            },
            connectionId,
            accountId: stringValue(request.config.accountId),
        ...
        if (!presentation) {
            throw new Error(`Google Slides presentation ${request.item.externalId} returned no data.`);
        }
        ...
        const sourceVersion = revisionId ??
        ...
        const items = normalizePresentationSlides(presentation, request.item, compiledSlides, sourceVersion);
...
function googleDriveFileInventoryItem(file) {
    const id = stringValue(file.id);
    if (!id)
        return null;
    return {
        ...
        canonicalUrl: stringValue(file.webViewLink) ??
...
export function googleSlidesRecommendedPresentationIds(config) {
    return stringArray(config.recommendedPresentationIds);
}
function normalizePresentationSlides(presentation, inventoryItem, compiledSlides, sourceVersion) {
    const slides = Array.isArray(presentation.slides) ? presentation.slides : [];
    const masters = objectMap(presentation.masters);
    const layouts = objectMap(presentation.layouts);
    const presentationTitle = stringValue(presentation.title) ?? inventoryItem.title;
    const revisionId = stringValue(presentation.revisionId);
    const compiledById = new Map(compiledSlides.map((compiled) => [compiled.objectId, compiled]));
    const normalizedSlides = slides.flatMap((value, index) => {
        ...
        const masterId = stringValue(properties?.masterObjectId) ??
        ...
        const canonicalBase = inventoryItem.canonicalUrl ??
            `https://docs.google.com/presentation/d/${encodeURIComponent(inventoryItem.externalId)}/edit`;
        const childItems = compiled.childArtifacts.map((child, childIndex) => normalizeContextItem({
            ...
            curationStatus: inventoryItem.upstreamAccess === "restricted" ? "review" : "included",
            provenance: {
                provider: "google-slides",
    ...
    const slideRoots = normalizedSlides.filter((item) => item.kind === "google-slides-slide");
    ...
    const parent = normalizeContextItem({
        ...
        canonicalUrl: inventoryItem.canonicalUrl ??
            `https://docs.google.com/presentation/d/${encodeURIComponent(inventoryItem.externalId)}/edit`,
...
function mediaWithNativeUrl(media, source, revisionId) {
...
async function fetchTemporarySlideRender(input) {
    const thumbnail = asRecord(await executeConnectorProviderRequest(input.context.providerApi, {
        provider: "google_slides",
        method: "GET",
...
function thumbnailCropRegion(input) {
    const left = Math.max(0, Math.min(input.imageWidth - 1, Math.floor((input.bounds.x / input.pageWidth) * input.imageWidth)));
    const top = Math.max(0, Math.min(input.imageHeight - 1, Math.floor((input.bounds.y / input.pageHeight) * input.imageHeight)));
    const right = Math.max(left + 1, Math.min(input.imageWidth, Math.ceil(((input.bounds.x + input.bounds.width) / input.pageWidth) *
        input.imageWidth)));
    const bottom = Math.max(top + 1, Math.min(input.imageHeight, Math.ceil(((input.bounds.y + input.bounds.height) / input.pageHeight) *
        input.imageHeight)));
    return { left, top, width: right - left, height: bottom - top };
}
function googleDimensionToPx(value) {
    if (!value)
        return 0;
...
async function hydrateSlideThumbnails(input) {
    if (input.config.hydrateThumbnails === false)
        return [];
    const candidates = input.items.filter((item) => Boolean(stringValue(item.metadata?.slideObjectId)));
    const limit = Math.min(candidates.length, positiveLimit(input.config.thumbnailLimit, 15, 50));
    const warnings = [];
    for (let index = 0; index < limit; index++) {
        ...
        try {
            ...
            if (!width || !height || width > 400 || height > 400) {
                warnings.push(`Slide ${slideObjectId} UI thumbnail exceeded the 400px dimension limit.`);
                continue;
            }
            const remote = await fetchRemoteArtifact(contentUrl, input.context);
            if (remote.data.byteLength > 512 * 1024) {
                warnings.push(`Slide ${slideObjectId} UI thumbnail exceeded the 512 KiB storage limit.`);
                continue;
            }
            const stored = await storePrivateArtifact({
                data: remote.data,
            ...
            if (isContextConnectorQuotaError(error))
                throw error;
            warnings.push(`Slide ${slideObjectId} thumbnail was skipped: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    return warnings;
}
...
function inheritedSlideTheme(master, layout, slide) {
    const merged = deepMerge({}, asRecord(master?.pageProperties) ?? {}, asRecord(layout?.pageProperties) ?? {}, asRecord(slide.pageProperties) ?? {});
    const resolvedTypography = resolveInheritedSlideTypography(master, layout, slide);
    return {
...
function resolveInheritedSlideTypography(master, layout, slide) {
    ...
    for (const element of [...slideElements.values()].slice(0, 200)) {
        ...
        const layoutElement = resolvePlaceholderParent(slidePlaceholder, layoutElements, layoutPlaceholders);
        const layoutPlaceholder = pageElementPlaceholder(layoutElement);
        const masterElement = resolvePlaceholderParent(layoutPlaceholder, masterElements, masterPlaceholders);
        const resolvedStyle = deepMerge({}, pageElementTextStyle(masterElement), pageElementTextStyle(layoutElement), pageElementTextStyle(element));
        ...
        if (resolvedTextStyles.length >= 100)
            break;
    }
    const themeEvidence = {
        placeholders,
...
function placeholderElementMap(elements) {
...
function resolvePlaceholderParent(placeholder, elements, placeholders) {
    if (!placeholder)
        return undefined;
...
function pageElementPlaceholder(element) {
    const shape = asRecord(element?.shape);
    return asRecord(shape?.placeholder);
}
function pageElementTextStyle(element) {
    const shape = asRecord(element?.shape);
    ...
    for (const value of textElements) {
        ...
        deepMerge(style, asRecord(paragraphMarker?.style) ?? {}, asRecord(textRun?.style) ?? {}, asRecord(autoText?.style) ?? {});
    }
    return style;
}
function boundedThemeObject(value, depth = 0) {
    if (depth >= 5)
        return {};
...
function collectThemeStrings(value, key, limit) {
    const values = new Set();
    visitTheme(value, (currentKey, currentValue) => {
        if (currentKey !== key)
            return;
        const text = stringValue(currentValue);
        if (text && values.size < limit)
            values.add(text);
    });
    return [...values];
}
function collectThemeFontSizes(value, limit) {
    const values = new Set();
    visitTheme(value, (key, currentValue) => {
        if (key !== "fontSize" || values.size >= limit)
            return;
...
function collectThemeColors(value, limit) {
    const values = new Set();
    visitTheme(value, (key, currentValue) => {
...
function visitTheme(value, visit, depth = 0) {
    if (depth >= 8 || !value)
        return;
    if (Array.isArray(value)) {
...
function googleDriveFileAccess(file) {
    ...
    if (file.copyRequiresWriterPermission === true ||
        ...
        permissions.some((permission) => permission.deleted === true ||
...
function googleDriveAccessSignals(file) {
    return {
        ownedByMe: typeof file.ownedByMe === "boolean" ? file.ownedByMe : null,
        shared: typeof file.shared === "boolean" ? file.shared : null,
        copyRequiresWriterPermission: typeof file.copyRequiresWriterPermission === "boolean"
...
function googleDriveContainerIdFromUrl(value, label) {
