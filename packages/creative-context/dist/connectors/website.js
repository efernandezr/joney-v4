...
import { extractStaticWebsiteContext, readBoundedResponseBytes, } from "@agent-native/core/ingestion";
import { normalizeContextItem } from "./normalize.js";
import { fetchRemoteArtifact, sanitizeProvenanceUrl, sanitizeRemoteArtifact, storePrivateArtifact, } from "./private-artifacts.js";
import { asRecord, cursorOffset, positiveLimit, stringArray, stringValue, } from "./provider-response.js";
import { boundWebsiteExtraction, LayeredRenderedPageProvider, } from "./rendered-page.js";
export class WebsiteContextConnector {
    kind = "website";
    label = "Websites";
    supportsIncremental = true;
    #defaultRenderer = new LayeredRenderedPageProvider();
    async inventory(request, context) {
        const discovered = await discoverWebsiteInventory(request.config, context.signal);
    ...
    async fetch(request, context) {
        ...
        const screenshotArtifacts = await Promise.all(screenshots.map(async (screenshot) => ({
            screenshot,
        ...
        const rehostedAssets = await rehostUsefulWebsiteAssets(rawExtraction.assets, rawRendered.finalUrl, request.config, context);
        return {
            items: [
                normalizeContextItem({
                    ...
                    sourceModifiedAt: typeof rendered.metadata.lastModified === "string"
                        ? rendered.metadata.lastModified
                        : undefined,
                    sourceVersion: typeof rendered.metadata.etag === "string"
...
async function rehostUsefulWebsiteAssets(assets, finalUrl, config, context) {
    if (config.rehostAssets === false)
    ...
    const ranked = [
        ...new Map(assets
            ...
            .map((asset) => [new URL(asset.url, finalUrl).href, asset])).entries(),
    ]
    ...
    for (const asset of ranked) {
        try {
            ...
            if (!mimeType || !mimeMatchesKind(mimeType, asset.kind)) {
                warnings.push(`Skipped website asset with unexpected MIME ${mimeType ?? "unknown"}: ${asset.url}`);
                continue;
            }
            ...
            const stored = await storePrivateArtifact({
                ...
                metadata: {
                    ...
                    canonicalLogoCandidate: asset.role === "logo" || isLogoLikeAsset(asset.url),
                },
            });
            media.push({
            ...
            warnings.push(`Website asset could not be privately rehosted: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    return { media, warnings };
}
function sanitizeUrlLikeStrings(value) {
    return value.replace(/https?:\/\/[^\s"'<>]+/gi, (candidate) => sanitizeProvenanceUrl(candidate));
}
function sanitizeUrlValues(value) {
    if (typeof value === "string") {
        return sanitizeUrlLikeStrings(value);
    }
    if (Array.isArray(value)) {
        return value.map((entry) => sanitizeUrlValues(entry));
    }
...
function websiteMediaKind(kind) {
...
function isLogoLikeAsset(url) {
    return /(?:^|[/_\-.])(logo|brand|wordmark|logomark|favicon|og[-_]?image|icon)(?:[/_\-.]|$)/i.test(new URL(url).pathname);
}
function mimeMatchesKind(mimeType, kind) {
    return mimeType.startsWith(`${kind}/`);
}
function websiteAssetFilename(url, mimeType) {
...
function serializeRenderedSnapshot(value) {
...
function boundedEntry(value, depth) {
    if (typeof value === "string")
...
export async function discoverWebsiteInventory(config, signal, fetcher = ssrfSafeFetch) {
    ...
    const explicitSitemaps = stringArray(config.sitemapUrls).map(normalizeWebsiteUrl);
    ...
    if (references.size === 0 &&
        ...
        throw new Error("Website connector config requires explicit urls, domains/baseUrls, or sitemapUrls.");
    }
    const maxPages = positiveLimit(config.maxPages, 100, 1_000);
    const maxBytes = positiveLimit(config.maxInventoryBytes, 5 * 1024 * 1024, 20 * 1024 * 1024);
    const deadline = Date.now() + positiveLimit(config.maxInventoryMs, 15_000, 60_000);
    let consumedBytes = 0;
    ...
    for (const origin of allowedOrigins) {
        ...
        const robots = await fetchRobots(origin, signal, fetcher, maxBytes - consumedBytes).catch(() => ({
    ...
    while (sitemapQueue.length > 0 &&
        ...
        if (seenSitemaps.has(sitemap))
            continue;
        seenSitemaps.add(sitemap);
        if (!allowedOrigins.has(new URL(sitemap).origin))
            continue;
        const response = await fetcher(sitemap, { signal }, { maxRedirects: 5 }).catch(() => null);
        if (!response?.ok)
            continue;
        const bytes = await readBoundedResponseBytes(response, Math.max(1, maxBytes - consumedBytes));
        ...
        for (const raw of xml.matchAll(/<loc\b[^>]*>([\s\S]*?)<\/loc>/gi)) {
            ...
            if (policy && isRobotsDisallowed(url, policy))
                continue;
            references.set(url, { url, title: titleFromWebsiteUrl(url) });
            if (references.size >= maxPages)
                break;
        }
    }
    for (const root of roots) {
        ...
        const response = await fetcher(root, { signal }, { maxRedirects: 5 }).catch(() => null);
        if (!response?.ok)
            continue;
        const bytes = await readBoundedResponseBytes(response, Math.max(1, maxBytes - consumedBytes));
        consumedBytes += bytes.byteLength;
        const extraction = extractStaticWebsiteContext(new TextDecoder().decode(bytes), response.url || root);
        const policy = robotsByOrigin.get(new URL(root).origin);
        for (const url of extraction.internalLinks) {
            inspected++;
            if (sameOrigin(root, url) &&
    ...
    return {
        references: [...references.values()]
            .sort((a, b) => websitePriority(b.url) - websitePriority(a.url) ||
...
async function fetchRobots(origin, signal, fetcher, maxBytes) {
    const response = await fetcher(`${origin}/robots.txt`, { signal }, { maxRedirects: 5 });
    if (!response.ok) {
...
function isRobotsDisallowed(value, policy) {
    const path = `${new URL(value).pathname}${new URL(value).search}`;
    return policy.disallow.some((prefix) => prefix !== "/" ? path.startsWith(prefix) : true);
}
function normalizeWebsiteRoot(value) {
    return normalizeWebsiteUrl(/^https?:\/\//i.test(value) ? value : `https://${value}`);
}
function normalizeDiscoveredUrl(value, base) {
    try {
...
function titleFromWebsiteUrl(value) {
    const url = new URL(value);
    const part = url.pathname.split("/").filter(Boolean).at(-1);
    return part ? decodeURIComponent(part).replace(/[-_]+/g, " ") : url.hostname;
}
function websitePriority(value) {
    const path = new URL(value).pathname.toLowerCase();
    if (path === "/")
        return 100;
    if (/\/(brand|about|product|features|customers|case-stud|pricing)(\/|$)/.test(path))
        return 80;
    if (/\/(docs?|blog|news|legal|privacy|terms)(\/|$)/.test(path))
        return 30;
    return 50;
}
