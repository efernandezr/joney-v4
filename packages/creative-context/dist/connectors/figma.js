...
export class FigmaContextConnector {
    kind = "figma";
    label = "Figma";
    supportsIncremental = true;
    async inventory(request, context) {
        const connectionId = await connectorConnectionId("figma", request.config, context.resolveConnection);
        const references = await figmaReferences(request.config, connectionId, context);
    ...
    async fetch(request, context) {
        const connectionId = await connectorConnectionId("figma", request.config, context.resolveConnection);
        const sourceUrl = request.item.canonicalUrl ??
...
async function figmaReferences(config, connectionId, context) {
    ...
    for (const value of records) {
        ...
        const key = stringValue(record?.key) ?? (url ? figmaKeyFromUrl(url) : undefined);
        if (!key)
            continue;
        found.set(key, {
            key,
    ...
    const projectIds = new Set([
        ...
        ...stringArray(config.projectUrls).map((url) => figmaContainerIdFromUrl(url, "project")),
    ]);
    for (const id of stringArray(config.teamIds)) {
        for (const projectId of await figmaProjectIdsForTeam(id, connectionId, context)) {
    ...
    for (const projectId of projectIds) {
        for (const file of await figmaFilesForProject(projectId, connectionId, context)) {
            found.set(file.key, file);
        }
    }
    if (found.size === 0) {
        throw new Error("Figma connector config requires explicit file, project, or team IDs/URLs. The Figma API does not provide a global file inventory.");
    }
    return [...found.values()];
}
export function figmaRecommendedFileKeys(config) {
    return stringArray(config.recommendedFileKeys);
}
async function figmaProjectIdsForTeam(teamId, connectionId, context) {
    ...
    return (Array.isArray(payload?.projects) ? payload.projects : []).flatMap((value) => {
        const id = stringValue(asRecord(value)?.id);
        return id ? [id] : [];
    });
}
async function figmaFilesForProject(projectId, connectionId, context) {
    const payload = asRecord(await executeConnectorProviderRequest(context.providerApi, {
        provider: "figma",
        method: "GET",
    ...
    return (Array.isArray(payload?.files) ? payload.files : []).flatMap((value) => {
...
function figmaContainerIdFromUrl(value, kind) {
...
function figmaKeyFromUrl(value) {
    const url = new URL(value);
    if (url.hostname !== "figma.com" && !url.hostname.endsWith(".figma.com")) {
