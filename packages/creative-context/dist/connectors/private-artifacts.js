...
import { extractDominantColors, fingerprintMedia, readBoundedResponseBytes, } from "@agent-native/core/ingestion";
import { putPrivateBlob as corePutPrivateBlob, readPrivateBlob as coreReadPrivateBlob, } from "@agent-native/core/private-blob";
...
export async function storePrivateArtifact(input) {
    ...
    if (!handle) {
        throw new Error("Private blob storage is required for creative context artifacts.");
    }
...
export async function rehostRemoteMedia(input) {
    ...
    return {
        ...
        provenanceUrl: sanitizeProvenanceUrl(input.provenanceUrl ?? remote.finalUrl),
...
export async function fetchRemoteArtifact(url, context) {
    const response = await ssrfSafeFetch(url, { signal: context.signal }, { maxRedirects: 5 });
    if (!response.ok) {
...
export function sanitizeProvenanceUrl(value) {
    try {
...
export function parsePrivateBlobHandle(value) {
    ...
    try {
        const parsed = JSON.parse(Buffer.from(value.slice(PRIVATE_BLOB_REF_PREFIX.length), "base64url").toString("utf8"));
...
//# sourceMappingURL=private-artifacts.js.map