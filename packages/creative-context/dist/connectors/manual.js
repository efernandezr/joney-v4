...
export class ManualContextConnector {
    kind = "manual";
    label = "Manual text";
...
function parseManualEntries(config) {
    ...
    return config.items.map((value, index) => {
        ...
        if (!text)
            throw new Error(`Manual context ${id} requires text.`);
        if (text.length > 250_000) {
            throw new Error(`Manual context ${id} is too large for inline text; use the upload connector.`);
        }
        return {
            id,
            title,
...
function findManualEntry(config, inventoryItem) {
