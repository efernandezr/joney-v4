...
export function normalizeContextItem(input) {
    const externalId = required(input.externalId, "externalId");
    const title = required(input.title, "title");
    const nativeArtifact = input.metadata &&
        ...
        ? input.metadata.nativeArtifact
        : null;
    const preserveContent = input.preserveContent === true ||
...
function requireBoundedNativeContent(value) {
    const bytes = Buffer.byteLength(value, "utf8");
    if (bytes > MAX_NATIVE_CONTENT_BYTES) {
        throw new Error(`Creative context native content is ${bytes} bytes and exceeds the ${MAX_NATIVE_CONTENT_BYTES}-byte SQL inline limit; split the artifact or store it in private blob storage instead of truncating it`);
    }
    return value;
}
export function assertContextItemSqlTextLimits(item) {
    const nativeArtifact = item.metadata?.nativeArtifact;
    const isNativeContent = item.mimeType === "text/html" &&
    ...
    assertSqlTextLimit(isNativeContent ? "native content" : "searchable content", item.content, contentLimit, isNativeContent
        ? "split the artifact or store it in private blob storage"
        : "normalize or chunk the source before ingest");
    if (item.summary) {
        assertSqlTextLimit("summary", item.summary, MAX_SUMMARY_BYTES, "normalize the summary before ingest");
    }
    assertJsonSqlTextLimit("item metadata", item.metadata, MAX_METADATA_BYTES, "move raw payloads to private blob storage before ingest");
    assertJsonSqlTextLimit("item provenance", item.provenance, MAX_METADATA_BYTES, "move raw payloads to private blob storage before ingest");
    let chunkBytes = 0;
    for (const chunk of item.chunks ?? []) {
        chunkBytes += Buffer.byteLength(chunk.text, "utf8");
        if (chunkBytes > MAX_SEARCHABLE_CONTENT_BYTES) {
            throw new Error(`Creative context chunks exceed the ${MAX_SEARCHABLE_CONTENT_BYTES}-byte SQL text limit; normalize or split the chunks before ingest`);
        }
        assertJsonSqlTextLimit("chunk metadata", chunk.metadata, MAX_METADATA_BYTES, "move raw payloads to private blob storage before ingest");
    }
    for (const media of item.media ?? []) {
        for (const [label, value] of [
            ...
            if (value) {
                assertSqlTextLimit(label, value, MAX_MEDIA_TEXT_BYTES, "move raw payloads to private blob storage before ingest");
            }
        }
        for (const [label, value] of [
            ["media URL", media.url],
            ...
            if (value.trim().toLowerCase().startsWith("data:")) {
                throw new Error(`Creative context ${label} cannot be an inline data URL; store the payload in private blob storage instead`);
            }
            assertSqlTextLimit(label, value, MAX_MEDIA_LOCATOR_BYTES, "move raw payloads to private blob storage before ingest");
        }
        assertJsonSqlTextLimit("media metadata", media.metadata, MAX_METADATA_BYTES, "move raw payloads to private blob storage before ingest");
    }
    for (const edge of item.edges ?? []) {
        assertJsonSqlTextLimit("edge metadata", edge.metadata, MAX_METADATA_BYTES, "move raw payloads to private blob storage before ingest");
    }
}
function assertJsonSqlTextLimit(label, value, maxBytes, guidance) {
    if (value === undefined)
        return;
...
function assertSqlTextLimit(label, value, maxBytes, guidance) {
    const bytes = Buffer.byteLength(value, "utf8");
    if (bytes > maxBytes) {
        throw new Error(`Creative context ${label} is ${bytes} bytes and exceeds the ${maxBytes}-byte SQL text limit; ${guidance}`);
    }
}
function boundChunks(chunks) {
            ...
            ? [
                {
                    ...chunk,
                    text,
                    endOffset: chunk.startOffset == null
                        ? chunk.endOffset
...
export function hashContextContent(value) {
    return createHash("sha256").update(stableJson(value)).digest("hex");
}
export function hashContextVersion(item) {
...
export function chunkContextText(input, maxChars = DEFAULT_CHUNK_CHARS) {
...
export function collectProviderText(value, options = {}) {
    const skip = new Set([
        "id",
        "url",
        "href",
    ...
    const visit = (current, key) => {
        if (values.join("\n").length >= maxChars || skip.has(key ?? ""))
            return;
        if (typeof current === "string") {
            const normalized = normalizeWhitespace(current);
            if (normalized && !looksLikeOpaqueId(normalized))
...
function canonicalUnordered(values, canonicalize) {
    return values
        .map(canonicalize)
        .sort((left, right) => stableJson(left) < stableJson(right)
        ? -1
        : stableJson(left) > stableJson(right)
            ? 1
            : 0);
}
function withoutVolatileCaptureMetadata(metadata) {
