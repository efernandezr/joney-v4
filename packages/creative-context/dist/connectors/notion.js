...
import { rehostRemoteMedia, sanitizeProvenanceUrl, } from "./private-artifacts.js";
import { asRecord, connectorConnectionId, cursorOffset, executeConnectorProviderRequest, positiveLimit, stringArray, stringValue, } from "./provider-response.js";
export class NotionContextConnector {
    kind = "notion";
    label = "Notion";
    supportsIncremental = true;
    async inventory(request, context) {
        const connectionId = await connectorConnectionId("notion", request.config, context.resolveConnection);
        ...
        while (items.length < limit && state.seen.length < maxPages) {
            ...
            if (state.seen.includes(candidate.id))
                continue;
            const page = asRecord(await executeConnectorProviderRequest(context.providerApi, {
                provider: "notion",
                method: "GET",
            ...
            if (!page)
                continue;
            const discovery = await fetchNotionBlocks(candidate.id, connectionId, context, positiveLimit(request.config.maxDiscoveryBlocksPerPage, 500, 2_000));
            state.seen.push(candidate.id);
            if (candidate.depth < maxDepth) {
                for (const child of discovery.childPages) {
                    if (state.seen.includes(child.id) ||
        ...
        const hasMore = state.seen.length < maxPages &&
    ...
    async fetch(request, context) {
        ...
        if (!page) {
            throw new Error(`Notion page ${request.item.externalId} returned no data.`);
        }
        const discovery = await fetchNotionBlocks(request.item.externalId, connectionId, context, positiveLimit(request.config.maxBlocks, 500, 2_000));
        ...
        const content = markdown.markdown ||
            collectProviderText({ properties: page.properties, blocks: indexableBlocks }, {
        ...
        return {
            items: [
                normalizeContextItem({
                    ...
                    content,
                    chunks,
                    sourceModifiedAt: stringValue(page.last_edited_time) ?? request.item.sourceModifiedAt,
                    sourceVersion: stringValue(page.last_edited_time),
                    parseStatus: "parsed",
                    upstreamAccess: notionPageAccess(page),
                    curationStatus: notionPageAccess(page) === "restricted" ? "review" : "included",
                    provenance: {
                        provider: "notion",
                    ...
                    metadata: {
                        ...
                        selectedRootPageId: stringValue(request.item.metadata?.selectedRootPageId) ??
                            request.item.externalId,
                        parentPageId: stringValue(request.item.metadata?.parentPageId) ?? null,
                            ...
                            ? [
                                {
                                    ...
                                    toExternalId: stringValue(request.item.metadata?.parentPageId),
                                },
                            ]
                            : []),
                    ],
...
function sanitizeNotionBlockMediaUrl(value) {
    const block = asRecord(value);
    const type = stringValue(block?.type);
    if (!block ||
        !type ||
        !["image", "video", "audio", "file", "pdf"].includes(type)) {
        return value;
    }
...
function parseNotionInventoryCursor(value) {
    if (!value)
        return { rootIndex: 0, queue: [], seen: [] };
    if (/^\d+$/.test(value)) {
...
function encodeNotionInventoryCursor(state) {
    return Buffer.from(JSON.stringify(state)).toString("base64url");
}
function isNotionInventoryCandidate(value) {
    const candidate = asRecord(value);
    return Boolean(stringValue(candidate?.id) &&
...
export function notionRecommendedRootPageIds(config) {
    return stringArray(config.recommendedRootPageIds);
}
function notionBoundaryRoots(config) {
    const roots = new Map();
    ...
    for (const value of Array.isArray(config.roots) ? config.roots : []) {
        ...
        if (!id)
            continue;
        const boundary = root?.boundary === "teamspace-root" ? "teamspace-root" : "root-page";
        roots.set(id, {
            id,
            boundary,
    ...
    if (roots.size === 0) {
        throw new Error("Notion connector config requires rootPageIds or teamspaceRootPageIds. Search recommendations are not an import boundary.");
    }
    return [...roots.values()];
}
function notionPageIdFromUrl(value) {
    ...
    if (!compact)
        throw new Error(`Could not find a Notion page ID in ${value}.`);
    return [
        compact.slice(0, 8),
        compact.slice(8, 12),
        compact.slice(12, 16),
...
function notionHeadingChunks(sections) {
...
async function rehostNotionMedia(blocks, context) {
    const candidates = [];
    ...
    return Promise.all(candidates.slice(0, 50).map((candidate) => rehostRemoteMedia({
        ...candidate,
        context,
        metadata: { provider: "notion", blockId: candidate.blockId },
    })));
}
async function fetchNotionBlocks(rootId, connectionId, context, maxBlocks) {
    const blocks = [];
...
function notionPageAccess(page) {
    if (page.archived === true || page.in_trash === true)
        return "restricted";
    if (stringValue(page.public_url))
        return "available";
    return "unknown";
}
