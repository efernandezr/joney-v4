import type { PrivateBlobHandle, PrivateBlobPutInput, PrivateBlobReadResult } from "@agent-native/core/private-blob";
import type { ProviderApiRuntime } from "@agent-native/core/provider-api";
import type { ContextSourceKind, NormalizedContextItem, UpstreamAccess } from "../types.js";
...
export interface ContextConnectorExecutionContext {
    ...
    signal?: AbortSignal;
    now?: () => Date;
    resolveConnection?: (provider: string, requestedConnectionId?: string) => Promise<string | undefined>;
    loadUpload?: (item: ContextConnectorInventoryItem, config: Record<string, unknown>) => Promise<ContextUploadContent>;
    putPrivateBlob?: (input: PrivateBlobPutInput) => Promise<PrivateBlobHandle | null>;
    readPrivateBlob?: (handle: PrivateBlobHandle) => Promise<PrivateBlobReadResult>;
}
