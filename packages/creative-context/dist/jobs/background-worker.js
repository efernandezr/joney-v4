...
import { enrichCreativeContextMedia, projectCreativeContextMedia, } from "../server/enrichment.js";
import { claimJobLease, createDailyMaintenanceJob, createJob, getJob, inferBrandDnaProposalFromCorpus, listDueContextBackgroundJobDispatches, listContextSourcesDueForMaintenance, listAccessibleSearchDocuments, proposeCreativeContextSuggestion, purgeContextSourceArtifacts, releaseJobLease, updateLeasedJob, } from "../store/index.js";
...
export function registerCreativeContextBackgroundDispatcher(dispatcher) {
...
export async function processDueCreativeContextBackgroundJobs(input) {
    ...
    for (const job of due) {
        try {
            ...
            failed += 1;
            console.error(`[creative-context] failed to dispatch background job ${job.jobId}:`, error instanceof Error ? error.message : error);
        }
    }
...
export async function enqueueCreativeContextDailyMaintenance(input) {
    ...
    for (const source of due) {
        try {
            ...
            failed += 1;
            console.error(`[creative-context] maintenance enqueue ${source.sourceId} failed:`, error instanceof Error ? error.message : error);
        }
    }
    return { discovered: due.length, queued, failed };
}
export async function processCreativeContextBackgroundJob(input) {
    return runWithRequestContext({
        ...
        try {
            ...
            const progressCurrent = claimed.progressCurrent + Number(outcome.progress ?? 1);
...
async function processClaimedJob(job) {
    if (job.kind === "embed") {
        ...
        const batch = operation === "rebuild-fts"
            ? await rebuildFtsBatch(job)
            : operation === "rebuild-embeddings"
                ? await rebuildVectorBatch(job)
                : null;
        if (!batch) {
            throw new Error(`Unsupported embedding job: ${typeof operation === "string" ? operation : (JSON.stringify(operation) ?? "unknown")}`);
        }
        return {
    ...
    if (job.kind === "enrich-media") {
        ...
        const completed = new Set(Array.isArray(job.checkpoint?.completedMediaIds)
            ? job.checkpoint.completedMediaIds.filter((entry) => typeof entry === "string")
            : []);
        const limit = Math.max(1, Math.min(100, Number(job.budget?.eagerLimit ?? 25)));
        const pending = mediaIds.filter((mediaId) => !completed.has(mediaId));
        const results = [];
        for (const mediaId of pending.slice(0, limit)) {
            results.push(job.request.operation === "project-media"
    ...
    if (job.kind === "canonical-logo") {
        ...
        const candidates = [
            ...new Map(documents.map((document) => [document.itemId, document])).values(),
        ]
            .filter((document) => /logo|wordmark|brandmark|logomark/i.test(document.title) ||
            /logo|vector|component|image|asset/i.test(document.kind))
            .sort((left, right) => Number(right.curationRank === "canonical") -
        ...
        for (const [rank, candidate] of candidates.entries()) {
            suggestions.push(await proposeCreativeContextSuggestion({
                kind: "canonical-logo",
                profileId: optionalText(job.request.profileId),
    ...
    if (job.kind === "layout-suggestion") {
        ...
        const candidates = [
            ...
            .filter((document) => document.priorReuseCount >= 2 ||
            document.curationRank === "canonical" ||
            document.curationRank === "exemplar")
            .sort((left, right) => right.priorReuseCount - left.priorReuseCount ||
            left.itemId.localeCompare(right.itemId))
            .slice(0, Math.max(1, Math.min(25, Number(job.budget?.eagerLimit ?? 10))));
        const suggestions = [];
    ...
    if (job.kind === "purge") {
        if (!job.sourceId)
            throw new Error("Purge job requires a source id.");
...
function requestedMediaIds(job) {
    const many = Array.isArray(job.request.mediaIds)
        ? job.request.mediaIds.filter((entry) => typeof entry === "string" && entry.length > 0)
        : [];
    const single = optionalText(job.request.mediaId);
    return [...new Set([...many, ...(single ? [single] : [])])];
}
function requestedItemIds(job) {
    if (!Array.isArray(job.request.itemIds))
        return undefined;
    const ids = job.request.itemIds.filter((entry) => typeof entry === "string" && entry.length > 0);
    return ids.length ? [...new Set(ids)] : undefined;
}
function optionalText(value) {
    return typeof value === "string" && value.trim() ? value.trim() : undefined;
}
async function localDispatcher(input) {
    const timer = setTimeout(() => {
        void processCreativeContextBackgroundJob(input).catch((error) => {
            console.error(`[creative-context] local background job ${input.jobId} failed:`, error instanceof Error ? error.message : error);
        });
