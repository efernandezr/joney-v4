...
import { awaitBootstrap, extractInternalBearerToken, fireInternalDispatch, FRAMEWORK_ROUTE_PREFIX, getH3App, isInBackgroundFunctionRuntime, readBody, verifyInternalToken, } from "@agent-native/core/server";
import { enqueueCreativeContextDailyMaintenance, processCreativeContextBackgroundJob, processDueCreativeContextBackgroundJobs, registerCreativeContextBackgroundDispatcher, } from "./background-worker.js";
import { processDueCreativeContextImportJobs, processCreativeContextImportJob, registerCreativeContextImportContinuationDispatcher, } from "./worker.js";
...
const delayedDispatches = new Map();
...
export function createCreativeContextWorkerPlugin(input) {
    ...
    return async (nitroApp) => {
        ...
        if (input.registerDispatcher !== false) {
            registerCreativeContextImportContinuationDispatcher((dispatch) => scheduleHostedDispatch({ ...dispatch, appId }));
        }
        registerCreativeContextBackgroundDispatcher((dispatch) => scheduleHostedBackgroundDispatch({ ...dispatch, appId }));
        ...
        h3App.use(CREATIVE_CONTEXT_IMPORT_PROCESSOR_ROUTE, async (event) => {
            if (event?.req?.method !== "POST") {
                return json({ error: "Method not allowed" }, 405);
            }
            const body = asDispatch(await readBody(event), appId);
            if (!body)
                return json({ error: "Invalid import dispatch" }, 400);
            const token = extractInternalBearerToken(event.req.headers?.get?.("authorization") ??
                event.node?.req?.headers?.authorization);
            if (!process.env.A2A_SECRET && !unsignedDispatchAllowed()) {
                return json({
                    error: "A2A_SECRET is required for hosted creative-context background processing.",
                }, 503);
            }
            ...
            try {
                ...
                console.error("[creative-context] hosted import processor failed:", error instanceof Error ? error.message : error);
                return json({ error: "Creative context import failed" }, 500);
            }
        });
        h3App.use(CREATIVE_CONTEXT_BACKGROUND_PROCESSOR_ROUTE, async (event) => {
            if (event?.req?.method !== "POST") {
                return json({ error: "Method not allowed" }, 405);
            }
            const body = asDispatch(await readBody(event), appId);
            if (!body)
                return json({ error: "Invalid background dispatch" }, 400);
            const token = extractInternalBearerToken(event.req.headers?.get?.("authorization") ??
                event.node?.req?.headers?.authorization);
            if (!process.env.A2A_SECRET && !unsignedDispatchAllowed()) {
                return json({
            ...
            try {
                ...
                console.error("[creative-context] hosted background processor failed:", error instanceof Error ? error.message : error);
    ...
    if (!maintenanceTimers.has(appId)) {
        const run = () => {
            void enqueueCreativeContextDailyMaintenance({ appId }).catch((error) => {
                console.error("[creative-context] daily maintenance enqueue failed:", error instanceof Error ? error.message : error);
            });
        };
        run();
        const timer = setInterval(run, Math.max(60_000, input.intervalMs ?? DEFAULT_MAINTENANCE_INTERVAL_MS));
        timer.unref?.();
...
function unsignedDispatchAllowed() {
    ...
    if (!existing) {
        ...
        const run = () => {
            ...
            void Promise.all([
                ...
                .catch((error) => {
                console.error("[creative-context] due job sweep failed:", error instanceof Error ? error.message : error);
            })
                .finally(() => {
                running = false;
            });
        ...
        run();
        const timer = setInterval(run, Math.max(1_000, input.intervalMs ?? DEFAULT_SWEEP_INTERVAL_MS));
        timer.unref?.();
...
async function scheduleHostedDispatch(dispatch) {
    ...
    if (delay > 0) {
        ...
        const timer = setTimeout(() => {
            ...
            void fireHostedDispatch(dispatch).catch((error) => {
                console.error(`[creative-context] delayed import dispatch ${dispatch.jobId} failed:`, error instanceof Error ? error.message : error);
            });
...
async function fireHostedDispatch(dispatch) {
...
async function scheduleHostedBackgroundDispatch(dispatch) {
    const delay = Math.max(0, resumeTime(dispatch) - Date.now());
    if (delay > 0) {
        ...
        const timer = setTimeout(() => {
            ...
            void fireInternalDispatch({
                ...
                console.error(`[creative-context] delayed background dispatch ${dispatch.jobId} failed:`, error instanceof Error ? error.message : error);
            });
        }, Math.min(delay, 2_147_000_000));
        timer.unref?.();
...
function asDispatch(value, appId) {
...
function resolveWaitUntil(event) {
    const candidate = event?.context?.cloudflare?.context?.waitUntil ??
