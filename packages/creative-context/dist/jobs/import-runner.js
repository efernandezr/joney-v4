...
import { assertInventoryComplete, consumeIngestionBudget, createIngestionBudgetState, createIngestionCheckpoint, ingestionBudgetStopReason, selectInventoryForHydration, } from "@agent-native/core/ingestion";
...
export async function runContextImportJob(options) {
    ...
    try {
        ...
        const connectorContext = {
            ...
            ownerEmail: options.connectorContext.ownerEmail ?? options.source.ownerEmail,
        };
        while (!checkpoint.inventoryComplete) {
            const stop = ingestionBudgetStopReason(limits, budget, now());
            if (stop) {
                const job = await yieldJob(options.persistence, claimed, token, checkpoint, stop);
            ...
            const page = await connector.inventory({
                ...
                syncCursor: claimed.mode === "incremental" ? options.source.syncCursor : null,
            ...
            checkpoint = appendInventory(checkpoint, page.items, {
                ...
                syncCursor: page.syncCursor ?? latestSourceModifiedAt(page.items) ?? undefined,
        ...
        if (!checkpoint.selectionApplied) {
            if (connector.verifiesContainerOwner?.({
                config: sourceConfig,
                inventory: checkpoint.inventory,
            }) === true &&
                options.persistence.markSourceContainerOwnerVerified) {
                await options.persistence.markSourceContainerOwnerVerified(options.source.id);
            }
            checkpoint = applyHydrationSelection(checkpoint, claimed.request, options.source.config, options.source.kind, new Date(now()), claimed.mode === "incremental" ? options.source.syncCursor : null);
            claimed = await requireLeasedUpdate(options.persistence, {
                jobId: claimed.id,
                leaseToken: token,
                patch: {
        ...
        while (checkpoint.itemOffset < checkpoint.inventory.length) {
            ...
            await renewLease(options.persistence, claimed.id, token, limits, now);
            const inventoryBatch = checkpoint.inventory.slice(checkpoint.itemOffset, checkpoint.itemOffset + limits.batchSize);
            const normalized = [];
            let failed = 0;
            for (const inventoryItem of inventoryBatch) {
                try {
                    ...
                    if (isContextConnectorQuotaError(error))
                        throw error;
                    if (options.continueOnItemError === false)
                        throw error;
                    failed++;
            ...
            const ingested = normalized.length > 0
            ...
            if (ingested.mediaIds.length > 0 &&
                options.persistence.enqueueMediaEnrichment) {
                await options.persistence.enqueueMediaEnrichment({
                    sourceId: options.source.id,
                    mediaIds: ingested.mediaIds,
                    eagerLimit: Math.min(ingested.mediaIds.length, finitePositive(claimed.request.eagerMediaLimit, 25)),
                });
            }
            checkpoint = {
                ...checkpoint,
                phase: "fetch",
                syncHighWater: latestHighWater(checkpoint.syncHighWater, latestSourceModifiedAt(normalized)),
            ...
            await progress.update({
                id: runId,
                owner: options.source.ownerEmail,
                percent: percent(checkpoint.itemsProcessed, checkpoint.inventory.length),
        ...
        const reconciliation = options.persistence.reconcileSourceInventory &&
            ...
            : null;
        const inference = options.persistence.inferBrandDnaProposal &&
            (checkpoint.created + checkpoint.versioned > 0 ||
                claimed.request.infer === true)
            ? await inferBrandDnaAfterImport(options.persistence, {
                sourceId: options.source.id,
                profileId: optionalString(claimed.request.profileId ?? options.source.config.profileId),
            })
            : undefined;
        if (checkpoint.syncHighWater) {
            await options.persistence.updateSourceSyncCursor?.(options.source.id, checkpoint.syncHighWater);
        }
        ...
        if (isContextConnectorQuotaError(error)) {
            ...
            await options.persistence.releaseJobLease({ jobId: claimed.id, token });
            if (!paused)
                throw new Error("Context import job lease was lost.");
            await progress.update({
                id: runId,
                owner: options.source.ownerEmail,
        ...
        await options.persistence.releaseJobLease({ jobId: claimed.id, token });
        if (!failed)
            throw new Error(`Import lease was lost: ${message}`);
        return { job: failed, yielded: false, reason: null };
    }
}
async function inferBrandDnaAfterImport(persistence, input) {
    try {
        ...
        console.error("[creative-context] post-import brand DNA inference failed:", errorMessage(error));
        return undefined;
    }
}
function parseCheckpoint(value) {
    ...
    const failures = Array.isArray(value.failures)
        ...
        : [];
    return {
        phase: value.phase === "fetch" || value.phase === "completed"
            ? value.phase
            : "inventory",
        ...
        syncHighWater: typeof value.syncHighWater === "string" ? value.syncHighWater : null,
    };
}
function applyHydrationSelection(checkpoint, request, sourceConfig, sourceKind, now, syncCursor) {
    ...
    const selectedExternalIds = explicitSelection ??
...
function appendInventory(checkpoint, page, options) {
    const inventory = new Map(checkpoint.inventory.map((item) => [item.externalId, item]));
    for (const item of page)
        inventory.set(item.externalId, item);
    if (inventory.size > options.maxItems) {
        throw new Error(`Context inventory exceeded the ${options.maxItems} item launch limit. Narrow the source or raise maxInventoryItems.`);
    }
    ...
    return {
        ...
        syncHighWater: latestHighWater(checkpoint.syncHighWater, options.syncCursor),
        inventoryComplete: options.complete,
        itemsInventoried: items.length,
        inventory: items,
    };
}
function latestSourceModifiedAt(items) {
    return items.reduce((latest, item) => latestHighWater(latest, item.sourceModifiedAt), null);
}
function latestHighWater(current, candidate) {
    if (!candidate)
        return current ?? null;
    if (!current)
        return candidate;
...
async function yieldJob(persistence, job, token, checkpoint, reason) {
...
async function renewLease(persistence, jobId, token, limits, now) {
...
async function requireLeasedUpdate(persistence, input) {
    const updated = await persistence.updateLeasedJob(input);
    if (!updated)
        throw new Error("Context import job lease was lost.");
    return updated;
}
function validateImportJob(job, sourceId) {
    if (!job)
        throw new Error("Context import job was not found.");
    if (job.kind !== "import")
...
function normalizeLimits(input) {
    return {
        ...
        inventoryPageSize: positive(input?.inventoryPageSize, DEFAULT_LIMITS.inventoryPageSize),
        maxInventoryItems: positive(input?.maxInventoryItems, DEFAULT_LIMITS.maxInventoryItems),
        leaseTtlMs: positive(input?.leaseTtlMs, DEFAULT_LIMITS.leaseTtlMs),
    };
}
function progressMetadata(jobId, sourceId, checkpoint) {
    return {
        jobId,
        sourceId,
        itemsInventoried: checkpoint.inventory.length,
