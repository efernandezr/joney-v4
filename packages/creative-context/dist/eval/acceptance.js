...
function condition(artifact, acceptanceCase) {
    const output = normalized(artifact.output);
    const required = unique(acceptanceCase.requiredTerms);
    const forbidden = unique(acceptanceCase.forbiddenTerms).filter((term) => output.includes(normalized(term)));
...
export function runContextAcceptanceEvaluation(input) {
    ...
    const cases = [...input.cases]
        ...
        const provenanceComplete = [...expected].every((key) => contextOn.provenanceKeys.includes(key));
        const optOutStructurallyClean = contextOff.usedContextKeys.length === 0 &&
    ...
    const contextOnWins = cases.filter((entry) => entry.winner === "context-on").length;
    const contextOffWins = cases.filter((entry) => entry.winner === "context-off").length;
    ...
    const summary = {
        ...
        contextOnRequiredTermCoverage: average(cases.map((entry) => entry.contextOn.requiredTermCoverage)),
        meanEditDistanceReduction: average(cases.map((entry) => {
        ...
        forbiddenOutputHits: cases.reduce((total, entry) => total + entry.contextOn.forbiddenHits.length, 0),
    ...
    if (summary.caseCount < thresholds.minimumCases) {
        failures.push(`case count ${summary.caseCount} is below ${thresholds.minimumCases}`);
    }
    if (summary.contextOnWinRate < thresholds.minimumContextOnWinRate) {
        failures.push(`context-on win rate ${summary.contextOnWinRate.toFixed(3)} is below ${thresholds.minimumContextOnWinRate.toFixed(3)}`);
    }
    if (summary.contextOnRequiredTermCoverage <
        thresholds.minimumRequiredTermCoverage) {
        failures.push(`required-term coverage ${summary.contextOnRequiredTermCoverage.toFixed(3)} is below ${thresholds.minimumRequiredTermCoverage.toFixed(3)}`);
    }
    if (summary.meanEditDistanceReduction <
        thresholds.minimumMeanEditDistanceReduction) {
        failures.push(`edit-distance reduction ${summary.meanEditDistanceReduction.toFixed(3)} is below ${thresholds.minimumMeanEditDistanceReduction.toFixed(3)}`);
    }
    if (summary.forbiddenOutputHits)
        failures.push("forbidden output was emitted");
    if (summary.missingContextPacks)
        failures.push("context packs are missing");
    if (summary.missingProvenance)
        failures.push("provenance is incomplete");
...
export function assertContextAcceptanceGates(report) {
    const parsed = ContextAcceptanceReportSchema.parse(report);
    if (!parsed.gates.passed) {
        throw new Error(`Creative context acceptance gates failed: ${parsed.gates.failures.join("; ")}`);
    }
}
export function createBlindPreferencePacket(cases) {
    const worksheet = [];
    const answerKey = [];
    for (const entry of [...cases].sort((a, b) => a.id.localeCompare(b.id))) {
        const contextOnCandidate = parseInt(createHash("sha256").update(entry.id).digest("hex").slice(0, 2), 16) % 2
            ? "A"
            : "B";
        worksheet.push({
            caseId: entry.id,
            prompt: entry.prompt,
            candidateA: contextOnCandidate === "A"
                ? entry.contextOn.output
                : entry.contextOff.output,
            candidateB: contextOnCandidate === "B"
                ? entry.contextOn.output
                : entry.contextOff.output,
        });
...
export function scoreBlindPreferences(answerKey, scores) {
    let scoredTrials = 0;
    ...
    for (const entry of answerKey) {
        ...
        if (!score)
            continue;
        scoredTrials++;
        if (score === entry.contextOnCandidate)
            contextOnPreferences++;
    }
    return {
        scoredTrials,
