export { assertContextAcceptanceGates, ContextAcceptanceReportSchema, createBlindPreferencePacket, runContextAcceptanceEvaluation, scoreBlindPreferences, } from "./acceptance.js";
export { CREATIVE_CONTEXT_ACCEPTANCE_CASES } from "./acceptance-fixtures.js";
export { CREATIVE_CONTEXT_INK_IMAGE_BASE64, CREATIVE_CONTEXT_GOLD_DOCUMENTS, CREATIVE_CONTEXT_GOLD_TASKS, CREATIVE_CONTEXT_PURPLE_IMAGE_BASE64, } from "./fixtures.js";
...
const REQUIRED_CONSUMERS = ["slides", "assets", "design", "content"];
const REQUIRED_NATIVE_CONSUMERS = ["slides", "design"];
export function evaluateRankings(tasks, rankings) {
    if (tasks.length === 0) {
        return {
            taskCount: 0,
            top5Recall: 0,
    ...
    for (const task of tasks) {
        ...
        if (ranking.slice(0, 5).some((key) => relevant.has(key)))
            top5 += 1;
...
export function evaluateContextQuality(trials) {
    if (!trials.length) {
        return {
            trialCount: 0,
    ...
    const preferred = trials.filter((trial) => trial.preferred === "context-on").length;
    const reduction = trials.reduce((total, trial) => {
        if (trial.baselineEditDistance <= 0)
            return total;
        return (total +
...
export function creativeContextCompletionFailures(evidence) {
    const failures = [];
    if (evidence.permissionLeaks !== 0)
        failures.push("permission leaks");
    ...
    if (!evidence.importsResumable)
        failures.push("imports are not resumable");
    if (!evidence.importsIdempotent)
        failures.push("imports are not idempotent");
    ...
    const missingConnectors = REQUIRED_CONNECTORS.filter((connector) => !connectorSet.has(connector));
    ...
    const missingConsumers = REQUIRED_CONSUMERS.filter((consumer) => !consumerSet.has(consumer));
    ...
    const missingNativeRetrieval = REQUIRED_NATIVE_CONSUMERS.filter((consumer) => !nativeRetrievalSet.has(consumer));
    if (missingNativeRetrieval.length) {
        failures.push(`native code retrieval missing: ${missingNativeRetrieval.join(", ")}`);
    }
    const nativeFidelitySet = new Set(evidence.nativeCloneFidelityPassing);
    const missingNativeFidelity = REQUIRED_NATIVE_CONSUMERS.filter((consumer) => !nativeFidelitySet.has(consumer));
    if (missingNativeFidelity.length) {
        failures.push(`native clone fidelity missing: ${missingNativeFidelity.join(", ")}`);
    }
...
export function assertCreativeContextCompletionGates(evidence) {
    const failures = creativeContextCompletionFailures(evidence);
    if (failures.length) {
        throw new Error(`Creative context completion gates failed: ${failures.join("; ")}`);
    }
}
...
export async function bakeOffEmbeddingFamilies(input) {
    ...
    const allowed = input.allowedDocumentKeys
        ...
        : null;
    const results = await Promise.all(input.families.map(async (family) => {
        const documentVectors = await family.embed(documentEntries.map(([, value]) => value), "document");
        const queryVectors = await family.embed(input.tasks.map((task) => task.query), "query");
        const rankings = {};
    ...
    return results.sort((a, b) => a.metrics.permissionLeaks - b.metrics.permissionLeaks ||
        b.metrics.top5Recall - a.metrics.top5Recall ||
        b.metrics.meanReciprocalRank - a.metrics.meanReciprocalRank);
}
export function embeddingBakeoffPasses(metrics) {
    return (metrics.taskCount >= 6 &&
