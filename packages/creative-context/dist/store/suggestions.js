...
import { getCreativeContextItem, getCreativeContextItemByExternalId, } from "./content.js";
...
export async function listCreativeContextSuggestions(input = {}) {
...
async function loadOwnedSuggestion(suggestionId, kind) {
    const { getDb, schema } = getCreativeContext();
    const actor = requireActor();
    const rows = await getDb()
        .select()
        .from(schema.contextSuggestions)
        .where(and(eq(schema.contextSuggestions.id, suggestionId), eq(schema.contextSuggestions.kind, kind), eq(schema.contextSuggestions.ownerEmail, actor.ownerEmail)))
        .limit(1);
    const row = rows[0];
    if (!row)
        throw new Error("Suggestion not found or not accessible");
    if (row.profileId) {
        await assertAccess("creative-context-brand", row.profileId, "editor", undefined, { skipResourceBody: true });
    }
    return row;
}
export async function proposeCreativeContextSuggestion(input) {
    if (input.profileId) {
        await assertAccess("creative-context-brand", input.profileId, "editor", undefined, { skipResourceBody: true });
    }
    const detail = await getCreativeContextItem(input.itemId, input.itemVersionId);
    if (!detail)
        throw new Error("Suggestion item not found or not accessible");
    const { getDb, schema } = getCreativeContext();
    const actor = requireActor();
    const existing = await getDb()
        .select()
        .from(schema.contextSuggestions)
        .where(and(eq(schema.contextSuggestions.kind, input.kind), eq(schema.contextSuggestions.itemId, detail.item.id), eq(schema.contextSuggestions.itemVersionId, detail.version.id), eq(schema.contextSuggestions.ownerEmail, actor.ownerEmail)))
        .orderBy(desc(schema.contextSuggestions.createdAt))
        .limit(1);
    ...
    if (input.kind === "layout-template")
        delete payload.projectionItemId;
    const row = {
        id: newId("ccsg"),
        kind: input.kind,
        status: "proposed",
...
export async function decideCanonicalLogoSuggestion(input) {
    ...
    if (input.decision === "confirm") {
        ...
        if (!adapter) {
            throw new Error("Canonical-logo projection is not configured for this app; keep the proposal pending");
        }
        await adapter.apply({
            profileId: row.profileId ?? null,
            itemId: row.itemId,
...
export async function resolveLayoutProjectionItemId(suggestionId, candidateId) {
    if (typeof candidateId !== "string" || !candidateId)
        return null;
    const detail = await getCreativeContextItem(candidateId);
    if (!detail || detail.item.kind !== "layout_template")
        return null;
    const provenance = detail.item.provenance;
    return provenance.promotedFromSuggestionId === suggestionId
        ? detail.item.id
        : null;
}
async function loadCompiledLayoutSource(itemId, itemVersionId) {
    const detail = await getCreativeContextItem(itemId, itemVersionId);
    if (!detail)
        throw new Error("Layout source version is no longer accessible");
    const artifact = nativeCreativeArtifactFromMetadata(detail.version.metadata);
    if (!artifact ||
        artifact.app !== "slides" ||
        artifact.format !== "slides-html") {
        throw new Error("Layout promotion requires a compiled Slides-native Creative Context item");
    }
...
export async function applyLayoutTemplateSuggestion(input) {
    ...
    if (!adapter) {
        throw new Error("Layout-template projection is not configured for this app; keep the proposal pending");
    }
    let projectionItemId = await resolveLayoutProjectionItemId(row.id, payload.projectionItemId);
    const source = input.operation === "promote"
    ...
    const projectionItem = source
        ? {
            ...
            mimeType: "text/html",
            contentHash: await sha256(`${row.id}:${source.detail.version.id}:${source.compiled.html}`),
    ...
    if (input.operation === "promote" && !projectionItemId) {
        ...
        let projectionSource = sources.sources.find((candidate) => candidate.name === PROMOTED_LAYOUT_SOURCE);
        if (!projectionSource) {
        ...
        if (!projectionItemId)
            throw new Error("Failed to promote layout template");
    }
    if (input.operation === "promote") {
        ...
        if (projectionItemId) {
            await getDb()
                ...
                .where(and(eq(schema.contextItems.id, projectionItemId), eq(schema.contextItems.ownerEmail, actor.ownerEmail)));
        }
