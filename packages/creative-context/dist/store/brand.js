import { resourcePut, sharedResourceOwner, } from "@agent-native/core/resources/store";
...
async function loadDnaVersion(dnaVersionId) {
    if (!dnaVersionId)
        return null;
...
async function resolveEvidenceItems(evidenceItemIds) {
    ...
    const accessibleItems = await getDb()
        ...
        .where(and(inArray(schema.contextItems.id, evidenceIds), accessFilter(schema.contextSources, schema.contextSourceShares), ne(schema.contextSources.upstreamAccess, "restricted"), ne(schema.contextSources.status, "archived"), eq(schema.contextItems.curationStatus, "included"), ne(schema.contextItems.curationRank, "ignored"), eq(schema.contextItems.status, "active")));
    if (accessibleItems.length !== evidenceIds.length) {
        throw new Error("All brand DNA evidence items must be accessible");
    }
    return accessibleItems;
}
async function nextDnaVersionNumber(profileId) {
    const { getDb, schema } = getCreativeContext();
    const rows = await getDb()
...
export async function getBrandProfile(input) {
    const { getDb, schema } = getCreativeContext();
    let row;
    if (input.profileId) {
    ...
    if (!row)
        return { profile: null, dna: null, versions: [] };
    const versionRows = await getDb()
        ...
        .orderBy(desc(schema.brandDnaVersions.versionNumber))
        .limit(20);
    const versions = (await Promise.all(versionRows.map((version) => loadDnaVersion(version.id)))).filter((version) => Boolean(version));
    return {
...
async function projectPublishedBrandContext(profile, dna) {
    ...
    const resourceOwner = profile.visibility === "org" && profile.orgId
        ? sharedResourceOwner(profile.orgId)
        : actor.ownerEmail;
    await resourcePut(resourceOwner, "context/brand-context.md", content, "text/markdown", {
        createdBy: "user",
...
export async function previewBrandProfilePromotion(profileId) {
    const access = await assertAccess("creative-context-brand", profileId, "admin");
    const actor = requireActor();
    if (!actor.orgId || !(await currentRequestUserIsOrgAdmin(actor.orgId))) {
        throw new Error("Brand profile promotion requires an active organization owner or admin.");
    }
    ...
    if (!dna || dna.status !== "published") {
        throw new Error("Publish a reviewed brand DNA version before promoting its profile.");
    }
    return {
        profileId: profile.id,
...
export async function promoteBrandProfileToOrg(profileId, confirmation) {
    const preview = await previewBrandProfilePromotion(profileId);
    if (preview.profileName !== confirmation.profileName ||
        preview.dnaVersionId !== confirmation.dnaVersionId ||
        preview.targetOrgId !== confirmation.targetOrgId) {
        throw new Error("Brand profile changed after promotion preview; review and confirm it again.");
    }
    const actor = requireActor();
    const { getDb, schema } = getCreativeContext();
...
export async function findBrandProfileIdForInferenceSource(sourceId) {
    const { getDb, schema } = getCreativeContext();
    const rows = await getDb()
        .select({
    ...
    for (const row of rows) {
        ...
        const inference = payload.inference && typeof payload.inference === "object"
            ? payload.inference
            : null;
        if (inference?.sourceId === sourceId)
...
export async function publishBrandDna(input) {
    ...
    if (!proposal || proposal.status !== "proposed") {
        throw new Error("Published brand DNA must be an accessible proposed version");
    }
    if (proposal.profileId !== input.profileId) {
        throw new Error("Published proposal must belong to the selected profile");
    }
    if (input.confirmation.proposalVersionId !== proposal.id ||
        input.confirmation.contentHash !== proposal.contentHash) {
        throw new Error("Brand DNA changed after review; review and confirm the proposal again");
    }
    await getDb().transaction(async (tx) => {
        await tx
            .update(schema.brandProfiles)
...
export async function saveBrandDnaCandidate(input) {
    ...
    if (profileId) {
        const access = await assertAccess("creative-context-brand", profileId, "editor");
    ...
    await getDb().transaction(async (tx) => {
        ...
        if (accessibleItems.length) {
            await tx.insert(schema.brandDnaEvidence).values(accessibleItems.map((item) => ({
                id: newId("ccbe"),
                dnaVersionId,
                itemId: item.id,
    ...
    if (!dna)
        throw new Error("Failed to save brand DNA candidate");
    return { profile, dna };
}
