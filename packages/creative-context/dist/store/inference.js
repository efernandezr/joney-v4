import { findBrandProfileIdForInferenceSource, getBrandProfile, saveBrandDnaCandidate, } from "./brand.js";
import { getCreativeContextItem, listAccessibleSearchDocuments, } from "./content.js";
import { getContextSource } from "./sources.js";
function collectFonts(value, key = "", fonts = new Set()) {
...
function inferVoiceFeatures(details) {
    const text = details
    ...
    const headings = text
        .split("\n")
        .filter((line) => line.trim() && line.trim().split(/\s+/).length <= 8).length;
    const ctaCount = (text.match(/\b(?:start|try|build|launch|explore|learn|book)\b/gi) ?? []).length;
    ...
    return {
        descriptors: VOICE_DESCRIPTOR_ORDER.filter((descriptor) => descriptors.has(descriptor)),
        stats: {
...
function inferLayoutMotifs(details, documents) {
    const signal = [
...
function dnaSignals(payload) {
    const signals = new Set();
    const visit = (value, path) => {
        if (typeof value === "string") {
            const normalized = value.trim().toLowerCase().replace(/\s+/g, " ");
            if (normalized)
                signals.add(`${path}:${normalized}`);
            return;
        }
        ...
        if (value && typeof value === "object") {
            for (const [key, child] of Object.entries(value)) {
                if (/^(itemId|itemVersionId|thumbnailBlobRef)$/i.test(key))
                    continue;
...
export function brandDnaDriftScore(previous, candidate) {
...
function representativeScore(row) {
    return ((row.curationRank === "canonical"
        ? 100
...
export function selectRepresentativeBrandDocuments(documents, limit = 50) {
    const unique = [
...
export async function inferBrandDnaProposalFromCorpus(input) {
    ...
    if (!source)
        throw new Error("Context source not found or not accessible");
    const unique = selectRepresentativeBrandDocuments(documents, 50);
    if (!unique.length)
        return { proposal: null, reason: "no-hydrated-evidence" };
    const details = (await Promise.all(unique.map((row) => getCreativeContextItem(row.itemId, row.itemVersionId)))).filter((detail) => detail !== null);
    const colors = [
        ...new Set([
            ...unique.flatMap((row) => row.colors),
            ...details.flatMap((detail) => detail.media.flatMap((media) => media.palette)),
        ]),
    ].slice(0, 16);
    const fonts = [
        ...details.reduce((set, detail) => collectFonts(detail.version.metadata, "metadata", set), new Set()),
    ].slice(0, 12);
    ...
    const confidence = Math.min(0.95, 0.35 +
        Math.min(0.3, unique.length / 50) +
        (colors.length ? 0.15 : 0) +
        (fonts.length ? 0.1 : 0) +
        (layoutPatterns.length ? 0.05 : 0));
    const configuredProfileId = typeof source.config.profileId === "string" && source.config.profileId
    ...
    const threshold = Math.max(0, Math.min(1, input.materialDriftThreshold ?? BRAND_DNA_MATERIAL_DRIFT_THRESHOLD));
