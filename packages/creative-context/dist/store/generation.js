...
import { assertGenerationArtifactAccessProof, } from "../server/generation-artifact-access.js";
...
export function assertGenerationCreativeContextInvariants(input) {
    ...
    for (const reference of references) {
        ...
        if (hasItemId !== hasItemVersionId) {
            throw new Error("Creative context provenance must provide both itemId and itemVersionId");
        }
        if (reference.influence !== "generated" && !hasItemId) {
            throw new Error("Only generated creative context provenance may omit item references");
        }
        if (!input.contextPackId && hasItemId) {
            throw new Error("Creative context provenance cannot reference items without a context pack");
        }
        if (input.contextMode === "off" &&
            (reference.influence !== "generated" || hasItemId)) {
            throw new Error("Creative context off records may contain only generated provenance without item references");
        }
    }
}
export async function recordGenerationCreativeContext(input, options = {}) {
    const elementProvenance = input.elementProvenance ??
    ...
    if (input.contextPackId) {
        await assertAccess("creative-context-pack", input.contextPackId, "viewer", undefined, { skipResourceBody: true });
    }
    if (input.contextPackId) {
        ...
        const allowed = new Set(members.map((member) => `${member.itemId}:${member.itemVersionId}`));
        for (const entry of elementProvenance) {
            if ((entry.itemId || entry.itemVersionId) &&
                (!entry.itemId ||
                    !entry.itemVersionId ||
                    !allowed.has(`${entry.itemId}:${entry.itemVersionId}`))) {
                throw new Error("Element provenance references must belong to the recorded context pack");
            }
        }
    }
    const { getDb, schema } = getCreativeContext();
    ...
    if (options.artifactAccess) {
        assertGenerationArtifactAccessProof(input, options.artifactAccess, "editor");
    }
    const row = {
        id: newId("ccgr"),
        appId: input.appId,
...
export async function getGenerationCreativeContext(input, options = {}) {
    ...
    if (options.artifactAccess && !actor.orgId) {
        throw new Error("Collaborative generation context reads require an organization");
    }
    if (options.artifactAccess) {
        assertGenerationArtifactAccessProof(input, options.artifactAccess, "viewer");
    }
    ...
    const rows = await getDb()
        ...
        .where(and(eq(schema.generationRecords.appId, input.appId), eq(schema.generationRecords.artifactType, input.artifactType), eq(schema.generationRecords.artifactId, input.artifactId), actorScope))
