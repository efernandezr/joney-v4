...
function actorOrgScope(schema, actor) {
    return actor.orgId
        ? eq(schema.contextJobs.orgId, actor.orgId)
        : isNull(schema.contextJobs.orgId);
}
async function findDeduplicatedJob(db, schema, actor, logicalKey) {
    const scope = tenantDedupeScope(actor);
    const scoped = await db
        .select()
        .from(schema.contextJobs)
        .where(and(eq(schema.contextJobs.dedupeScope, scope), eq(schema.contextJobs.scopedDedupeKey, logicalKey), eq(schema.contextJobs.ownerEmail, actor.ownerEmail), actorOrgScope(schema, actor)))
        .limit(1);
    if (scoped[0])
        return scoped[0];
    const legacy = await db
        .select()
        .from(schema.contextJobs)
        .where(and(eq(schema.contextJobs.dedupeKey, logicalKey), eq(schema.contextJobs.ownerEmail, actor.ownerEmail), actorOrgScope(schema, actor)))
        .limit(1);
...
export async function createJob(input) {
    ...
    if (input.dedupeKey) {
        const legacy = await findDeduplicatedJob(getDb(), schema, actor, input.dedupeKey);
        if (legacy)
            return mapJob(legacy);
        await getDb()
        ...
        const existing = await findDeduplicatedJob(getDb(), schema, actor, input.dedupeKey);
        if (existing)
    ...
    if (!created)
        throw new Error("Failed to create creative context job");
    return created;
}
export async function createDailyMaintenanceJob(input) {
    ...
    if (legacy)
        return { job: mapJob(legacy), created: false };
    const id = newId("ccj");
    ...
    if (!row)
        throw new Error("Failed to create scheduled maintenance job");
...
export async function enqueueContextRebuildJob(input) {
    ...
    const candidates = await getDb()
        .select()
        .from(schema.contextJobs)
        .where(and(eq(schema.contextJobs.ownerEmail, actor.ownerEmail), actorOrgScope(schema, actor), eq(schema.contextJobs.sourceId, input.sourceId), eq(schema.contextJobs.kind, "embed"), inArray(schema.contextJobs.status, ["queued", "paused"]), isNull(schema.contextJobs.leaseToken)))
        .limit(20);
    const existing = candidates.find((row) => parseJson(row.request, {}).operation ===
        input.operation);
    if (!existing) {
        return createJob({
            sourceId: input.sourceId,
    ...
    const request = parseJson(existing.request, {});
    const prior = Array.isArray(request.itemIds)
        ? request.itemIds.filter((item) => typeof item === "string")
        : [];
    ...
    if (!merged)
        throw new Error("Rebuild job was not accessible after merge.");
    return merged;
}
export async function getJob(jobId) {
    ...
    const rows = await getDb()
        .select()
        .from(schema.contextJobs)
        .where(and(eq(schema.contextJobs.id, jobId), eq(schema.contextJobs.ownerEmail, actor.ownerEmail), actorOrgScope(schema, actor)))
        .limit(1);
...
export async function listDueContextImportJobDispatches(input) {
    ...
    const rows = await getDb()
        ...
        .from(schema.contextJobs)
        .where(and(eq(schema.contextJobs.kind, "import"), or(eq(schema.contextJobs.status, "queued"), and(eq(schema.contextJobs.status, "paused"), or(isNull(schema.contextJobs.nextResumeAt), lte(schema.contextJobs.nextResumeAt, timestamp)))), or(isNull(schema.contextJobs.leaseToken), lt(schema.contextJobs.leaseExpiresAt, timestamp))))
...
export async function listDueContextBackgroundJobDispatches(input) {
    ...
    const rows = await getDb()
        ...
        .from(schema.contextJobs)
        .where(and(inArray(schema.contextJobs.kind, BACKGROUND_JOB_KINDS), or(eq(schema.contextJobs.status, "queued"), and(eq(schema.contextJobs.status, "paused"), or(isNull(schema.contextJobs.nextResumeAt), lte(schema.contextJobs.nextResumeAt, timestamp)))), or(isNull(schema.contextJobs.leaseToken), lt(schema.contextJobs.leaseExpiresAt, timestamp))))
        .limit(Math.max(1, Math.min(input.limit ?? 25, 100)));
    return rows.map((row) => ({
        jobId: row.jobId,
        ownerEmail: row.ownerEmail,
...
export async function updateJob(jobId, patch) {
    ...
    if (!current)
        throw new Error("Creative context job not found");
    ...
    if (!updated)
        throw new Error("Creative context job not found after update");
    return updated;
}
export async function claimJobLease(input) {
    ...
    await getDb()
        ...
        .where(and(eq(schema.contextJobs.id, input.jobId), eq(schema.contextJobs.ownerEmail, actor.ownerEmail), or(isNull(schema.contextJobs.leaseToken), lt(schema.contextJobs.leaseExpiresAt, timestamp)), or(eq(schema.contextJobs.status, "queued"), eq(schema.contextJobs.status, "running"), and(eq(schema.contextJobs.status, "paused"), or(isNull(schema.contextJobs.nextResumeAt), lte(schema.contextJobs.nextResumeAt, timestamp))))));
...
export async function renewJobLease(input) {
    ...
    await getDb()
        ...
        .where(and(eq(schema.contextJobs.id, input.jobId), eq(schema.contextJobs.ownerEmail, actor.ownerEmail), eq(schema.contextJobs.leaseToken, input.token), gt(schema.contextJobs.leaseExpiresAt, timestamp)));
    const renewed = await getJob(input.jobId);
    return (renewed?.leaseToken === input.token &&
...
export async function continueJob(jobId) {
    ...
    await getDb()
        ...
        .where(and(eq(schema.contextJobs.id, jobId), eq(schema.contextJobs.ownerEmail, actor.ownerEmail), or(isNull(schema.contextJobs.leaseToken), lt(schema.contextJobs.leaseExpiresAt, timestamp))));
...
export async function releaseJobLease(input) {
    ...
    if (current?.leaseToken !== input.token)
        return false;
    await getDb()
        .update(schema.contextJobs)
        .set({ leaseOwner: null, leaseToken: null, leaseExpiresAt: null })
        .where(and(eq(schema.contextJobs.id, input.jobId), eq(schema.contextJobs.ownerEmail, actor.ownerEmail), eq(schema.contextJobs.leaseToken, input.token)));
...
export async function updateLeasedJob(input) {
    ...
    await getDb()
        ...
        .where(and(eq(schema.contextJobs.id, input.jobId), eq(schema.contextJobs.ownerEmail, actor.ownerEmail), eq(schema.contextJobs.leaseToken, input.leaseToken), gt(schema.contextJobs.leaseExpiresAt, timestamp)));
