...
export function assertImmutablePackMembership(operation) {
    throw new Error(`Context-pack membership is append-only; ${operation} must derive a new pack snapshot`);
}
async function resolveBrandDnaVersion(dnaVersionId) {
    if (!dnaVersionId)
        return null;
    ...
    await assertAccess("creative-context-brand", rows[0].profileId, "viewer", undefined, { skipResourceBody: true });
    return dnaVersionId;
}
async function resolveMembers(members) {
    ...
    const accessibleItems = await getDb()
        ...
        .leftJoin(schema.creativeContextMemberships, and(eq(schema.creativeContextMemberships.publishedItemId, schema.contextItems.id), eq(schema.creativeContextMemberships.status, "active")))
        .leftJoin(schema.creativeContexts, eq(schema.creativeContexts.id, schema.creativeContextMemberships.contextId))
        .where(and(inArray(schema.contextItems.id, itemIds), or(and(accessFilter(schema.contextSources, schema.contextSourceShares), ne(schema.contextSources.upstreamAccess, "restricted"), ne(schema.contextSources.status, "archived")), accessFilter(schema.creativeContexts, schema.creativeContextShares)), eq(schema.contextItems.curationStatus, "included"), ne(schema.contextItems.curationRank, "ignored"), eq(schema.contextItems.status, "active")));
    if (accessibleItems.length !== itemIds.length) {
        throw new Error("All context-pack items must be accessible");
    }
    const itemById = new Map(accessibleItems.map((row) => [row.id, row]));
    const requestedVersionIds = members.flatMap((member) => member.itemVersionId ? [member.itemVersionId] : []);
    const versionRows = requestedVersionIds.length
        ? await getDb()
            ...
            .where(and(inArray(schema.contextItemVersions.id, requestedVersionIds), inArray(schema.contextItemVersions.itemId, itemIds)))
        : [];
    const versionOwner = new Map(versionRows.map((row) => [row.id, row.itemId]));
    return members.map((member) => {
        const item = itemById.get(member.itemId);
        const itemVersionId = member.itemVersionId ?? item.currentVersionId;
        if (member.itemVersionId &&
...
async function includeNativeArtifactDependencies(explicitMembers) {
    ...
    const initialItems = await getDb()
        ...
        .where(inArray(schema.contextItemVersions.id, explicitMembers.map((member) => member.itemVersionId)));
    ...
    while (frontier.length) {
        const edges = await getDb()
            ...
            .leftJoin(schema.contextItems, eq(schema.contextItems.id, schema.contextEdges.toItemId))
            .leftJoin(schema.contextItemVersions, and(eq(schema.contextItemVersions.id, schema.contextEdges.toItemVersionId), eq(schema.contextItemVersions.itemId, schema.contextEdges.toItemId)))
            .where(and(inArray(schema.contextEdges.fromItemVersionId, frontier), eq(schema.contextEdges.relation, "contains-native-child")))
            .orderBy(asc(schema.contextEdges.fromItemVersionId), asc(schema.contextEdges.toExternalId));
        const next = [];
        for (const edge of edges) {
            if (!edge.toItemId ||
                ...
                !edge.childSourceId ||
                !edge.childVersionId) {
                throw new Error(`Native artifact child ${edge.toExternalId ?? "(unknown)"} has no immutable version reference`);
            }
            const parentSourceId = sourceByVersion.get(edge.fromItemVersionId);
            if (!parentSourceId || parentSourceId !== edge.childSourceId) {
                throw new Error("Native artifact dependencies must stay within the pinned source snapshot");
            }
            const existing = byItemId.get(edge.toItemId);
            if (existing) {
                if (existing.itemVersionId !== edge.toItemVersionId) {
                    throw new Error("A native artifact dependency conflicts with an explicitly pinned item version");
                }
                continue;
            }
            dependencyCount += 1;
            if (dependencyCount > MAX_NATIVE_PACK_DEPENDENCIES) {
                throw new Error("Native artifact pack dependencies exceed the safe limit");
            }
            const dependency = {
...
export async function createContextPack(input) {
    ...
    const resolvedMembers = await includeNativeArtifactDependencies(explicitMembers);
    if (input.derivedFromPackId) {
        await assertAccess("creative-context-pack", input.derivedFromPackId, "viewer", undefined, { skipResourceBody: true });
    }
    ...
    await getDb().transaction(async (tx) => {
        ...
        if (resolvedMembers.length) {
            await tx.insert(schema.contextPackMembers).values(resolvedMembers.map((member, ordinal) => ({
                id: newId("ccpm"),
                packId: id,
                itemId: member.itemId,
        ...
        if (inheritedShares.length) {
            await tx.insert(schema.contextPackShares).values(inheritedShares.map((share) => ({
                id: newId("ccpsh"),
                resourceId: id,
...
export async function deriveContextPack(input) {
    ...
    return createContextPack({
        ...
        description: input.description === undefined ? base.description : input.description,
        derivedFromPackId: base.id,
        brandDnaVersionId: input.brandDnaVersionId === undefined
...
export async function setContextPackPinned(packId, pinned) {
    ...
    const existing = await getDb()
        ...
        .where(and(eq(schema.contextPackPins.packId, packId), eq(schema.contextPackPins.ownerEmail, actor.ownerEmail)))
        .limit(1);
    ...
    if (!pack)
        throw new Error("Context pack not found after pin update");
    return pack;
}
export async function archiveContextPack(packId) {
    ...
    const pack = await getContextPack(packId);
    if (!pack)
        throw new Error("Context pack not found after archive");
    return pack;
}
export async function listContextPacks(input) {
    ...
    if (input.cursor)
        filters.push(gt(schema.contextPacks.id, input.cursor));
    ...
    const [memberRows, pinRows] = ids.length
        ? await Promise.all([
            getDb()
                ...
                .innerJoin(schema.contextItems, eq(schema.contextItems.id, schema.contextPackMembers.itemId))
                .innerJoin(schema.contextSources, eq(schema.contextSources.id, schema.contextItems.sourceId))
                .leftJoin(schema.creativeContextPublishedSnapshots, and(eq(schema.creativeContextPublishedSnapshots.itemId, schema.contextPackMembers.itemId), eq(schema.creativeContextPublishedSnapshots.itemVersionId, schema.contextPackMembers.itemVersionId), eq(schema.creativeContextPublishedSnapshots.sourceId, schema.contextItems.sourceId)))
                .where(and(inArray(schema.contextPackMembers.packId, ids), or(and(accessFilter(schema.contextSources, schema.contextSourceShares), ne(schema.contextSources.upstreamAccess, "restricted"), ne(schema.contextSources.status, "archived")), isNotNull(schema.creativeContextPublishedSnapshots.id)), eq(schema.contextItems.curationStatus, "included"), ne(schema.contextItems.curationRank, "ignored"), eq(schema.contextItems.status, "active"))),
            getDb()
                .select({ packId: schema.contextPackPins.packId })
                .from(schema.contextPackPins)
                .where(and(inArray(schema.contextPackPins.packId, ids), eq(schema.contextPackPins.ownerEmail, actor.ownerEmail))),
        ])
        : [[], []];
    ...
    const pinned = new Set(pinRows.map((row) => row.packId));
    return {
        packs: page.map((row) => ({
            id: row.id,
            name: row.name,
...
export async function getContextPack(packId) {
    ...
    const [memberRows, pinRows] = await Promise.all([
        getDb()
            ...
            .innerJoin(schema.contextSources, eq(schema.contextSources.id, schema.contextItems.sourceId))
            .leftJoin(schema.creativeContextPublishedSnapshots, and(eq(schema.creativeContextPublishedSnapshots.itemId, schema.contextPackMembers.itemId), eq(schema.creativeContextPublishedSnapshots.itemVersionId, schema.contextPackMembers.itemVersionId), eq(schema.creativeContextPublishedSnapshots.sourceId, schema.contextItems.sourceId)))
            .where(and(eq(schema.contextPackMembers.packId, packId), or(and(accessFilter(schema.contextSources, schema.contextSourceShares), ne(schema.contextSources.upstreamAccess, "restricted"), ne(schema.contextSources.status, "archived")), isNotNull(schema.creativeContextPublishedSnapshots.id)), eq(schema.contextItems.curationStatus, "included"), ne(schema.contextItems.curationRank, "ignored"), eq(schema.contextItems.status, "active")))
    ...
    const members = memberRows.map((member) => ({
        ...
        reason: member.reason ?? null,
        score: member.score ?? null,
        scoreMetadata: (sanitizePublicMetadata(parseJson(member.scoreMetadata, {})) ?? {}),
    }));
    return {
        id: row.id,
        name: row.name,
