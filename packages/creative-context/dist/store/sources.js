...
function assertSafeSourceConfig(config) {
    ...
    while (pending.length) {
        ...
        for (const [key, child] of Object.entries(value)) {
            if (/(?:secret|token|password|api[-_]?key|credential|authorization)/i.test(key)) {
                throw new Error("Context source config may contain public settings only; store credentials in a connection and reference connectionId");
            }
            pending.push(child);
        }
    }
}
async function appendSourceAudit(tx, sourceId, operation, details) {
...
export function toSourceSummary(source) {
    const { id, name, kind, externalRef, connectionId, containerOwnerVerifiedAt, upstreamAccess, status, healthStatus, itemCount, restrictedItemCount, lastSyncedAt, lastError, visibility, createdAt, updatedAt, } = source;
    return {
        id,
        name,
        kind,
...
export function initialContextSourceHealth(kind, connectionId) {
    if (kind === "manual" || kind === "upload")
        return "healthy";
    if (kind === "website")
        return "stale";
...
export async function listContextSources(input) {
    ...
    if (input.kind)
        filters.push(eq(schema.contextSources.kind, input.kind));
    if (input.cursor)
        filters.push(gt(schema.contextSources.id, input.cursor));
...
export async function getContextSource(sourceId) {
    const access = await resolveAccess("creative-context-source", sourceId);
    return access ? mapSource(access.resource) : null;
}
export async function handleWorkspaceConnectionLifecycle(event) {
    const { getDb, schema } = getCreativeContext();
    const sources = await getDb()
        .select({ id: schema.contextSources.id })
        .from(schema.contextSources)
        .where(and(accessFilter(schema.contextSources, schema.contextSourceShares), eq(schema.contextSources.connectionId, event.connectionId), ne(schema.contextSources.status, "archived")));
    const sourceIds = sources.map((row) => row.id);
    if (!sourceIds.length)
        return { sources: 0, jobs: 0 };
...
export async function listContextSourcesDueForMaintenance(input) {
    ...
    const rows = await getDb()
        ...
        .from(schema.contextSources)
        .where(and(eq(schema.contextSources.status, "active"), inArray(schema.contextSources.kind, [
        "website",
        "figma",
        "notion",
        "google-slides",
    ]), or(isNull(schema.contextSources.lastSyncedAt), lt(schema.contextSources.lastSyncedAt, input.before))))
    ...
    const activeJobs = await getDb()
        ...
        .where(and(inArray(schema.contextJobs.sourceId, rows.map((row) => row.sourceId)), eq(schema.contextJobs.kind, "import"), inArray(schema.contextJobs.status, ["queued", "running", "paused"])));
    const busy = new Set(activeJobs.map((row) => row.sourceId));
    return rows
        .filter((row) => !busy.has(row.sourceId))
        .map((row) => ({
...
export async function createContextSource(input) {
    ...
    await getDb().transaction(async (tx) => {
        await tx.insert(schema.contextSources).values({
            ...
            containerOwnerVerifiedAt: input.kind === "manual" || input.kind === "upload" ? timestamp : null,
...
export async function updateContextSource(sourceId, patch) {
    ...
    if (patch.externalRef !== undefined)
        values.externalRef = patch.externalRef;
    ...
    if (patch.connectionId === null)
        values.healthStatus = "needs_setup";
    if (patch.syncCursor !== undefined)
        values.syncCursor = patch.syncCursor;
    if (patch.itemCount !== undefined)
        values.itemCount = patch.itemCount;
    ...
    if (!source)
        throw new Error("Context source not found after update");
    return source;
}
export async function archiveContextSource(sourceId) {
    return updateContextSource(sourceId, {
        status: "archived",
        healthStatus: "paused",
    });
}
export async function restoreContextSource(sourceId) {
    return updateContextSource(sourceId, {
        status: "active",
        healthStatus: "stale",
        lastError: null,
...
export async function promoteContextSource(sourceId, confirmation) {
    ...
    const source = await getContextSource(sourceId);
    if (!source)
        throw new Error("Context source not found after promotion");
    return source;
}
export function assertContextSourcePromotionConfirmation(preview, confirmation) {
    if (preview.containerRef !== confirmation.containerRef ||
        preview.boundaryHash !== confirmation.boundaryHash ||
        preview.itemCount !== confirmation.itemCount) {
        throw new Error("Source changed after promotion preview; review the container and item count again");
    }
}
const BOUNDARY_KEYS_BY_KIND = {
    "google-slides": [
        "presentationIds",
        "folderId",
        "folderUrl",
...
function safeBoundaryValue(value) {
    if (Array.isArray(value)) {
        ...
        return [
            ...new Map(canonical.map((entry) => [stringifyJson(entry), entry])).values(),
        ].sort((left, right) => stringifyJson(left).localeCompare(stringifyJson(right)));
    }
    if (value && typeof value === "object") {
        const selected = {};
        for (const key of Object.keys(value).sort()) {
            ...
            const child = safeBoundaryValue(value[key]);
            if (child !== undefined)
                selected[key] = child;
        }
...
export async function contextSourceBoundary(input) {
    ...
    const entries = Object.entries(selected).map(([key, value]) => `${key}: ${Array.isArray(value) ? value.join(", ") : String(value)}`);
    const summary = (typeof externalRef === "string" ? externalRef : null) ??
        entries.join("; ") ??
        `${input.kind} source`;
    return {
...
export async function previewContextSourcePromotion(sourceId) {
    const access = await assertAccess("creative-context-source", sourceId, "admin");
    const actor = requireActor();
    if (!actor.orgId) {
    ...
    const verifiedOwner = access.role === "owner" &&
        Boolean(access.resource.containerOwnerVerifiedAt);
    if (!orgAdmin && !verifiedOwner) {
        throw new Error("Source promotion requires an organization admin or verified container owner");
    }
...
export async function markSourceContainerOwnerVerified(sourceId) {
...
export async function deleteContextSource(sourceId) {
    ...
    const source = await getContextSource(sourceId);
    if (!source)
        throw new Error("Context source tombstone not found");
