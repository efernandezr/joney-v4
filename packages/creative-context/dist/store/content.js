import { buildSearchSnippet, escapeLikeTerm, normalizeSearchTerms, scoreSearchText, } from "@agent-native/core/search-utils";
import { currentRequestUserIsOrgAdmin } from "@agent-native/core/server";
import { accessFilter, assertAccess } from "@agent-native/core/sharing";
import { and, asc, count, desc, eq, gt, gte, inArray, isNotNull, lte, ne, or, sql, } from "drizzle-orm";
...
import { newId, nextOffsetCursor, nowIso, parseJson, parseOffsetCursor, requireActor, sha256, stringifyJson, } from "./helpers.js";
...
function mediaEnrichmentSourceContentHash(metadata, fallback) {
...
export function assertImmutableContextVersion(operation) {
    throw new Error(`Creative context item versions are immutable; ${operation} must create a new version`);
}
function isUniqueConstraintError(error) {
    ...
    for (let depth = 0; current && depth < 4; depth += 1) {
        ...
        const message = typeof candidate.message === "object"
...
export async function ingestItems(batch) {
    return ingestItemsAttempt(batch, true);
}
async function ingestItemsAttempt(batch, allowUniqueConflictRetry) {
    await assertAccess("creative-context-source", batch.sourceId, "editor", undefined, { skipResourceBody: true });
    ...
    const externalIds = Array.from(new Set(batch.items.map((item) => item.externalId)));
    if (externalIds.length !== batch.items.length) {
        throw new Error("An ingest batch cannot contain duplicate externalId values");
    }
    const existingRows = externalIds.length
        ? await getDb()
            .select()
            .from(schema.contextItems)
            .where(and(eq(schema.contextItems.sourceId, batch.sourceId), inArray(schema.contextItems.externalId, externalIds)))
        : [];
    const existingByExternalId = new Map(existingRows.map((row) => [row.externalId, row]));
    const currentVersionIds = existingRows.map((row) => row.currentVersionId);
    const currentVersionRows = currentVersionIds.length
        ...
        : [];
    const versionNumberById = new Map(currentVersionRows.map((row) => [row.id, row.versionNumber]));
    const sourceContentHashByVersionId = new Map(currentVersionRows.map((row) => {
    ...
    for (const input of batch.items) {
        ...
        if (existing &&
            ...
            const newlyRestricted = upstreamAccess === "restricted" &&
            ...
            itemUpdates.push({
                ...
                values: {
                    ...
                    colors: stringifyJson(input.colors ??
                        (input.color ? [input.color] : parseJson(existing.colors, []))),
                    provenance: stringifyJson(input.provenance ?? parseJson(existing.provenance, {})),
                    updatedAt: timestamp,
                },
            });
            unchanged += 1;
        ...
        if (existing) {
            ...
            itemUpdates.push({
                ...
                values: {
                    ...
                    colors: stringifyJson(input.colors ?? (input.color ? [input.color] : [])),
        ...
        const normalizedChunks = input.chunks?.length && input.chunks.length > 0
            ? input.chunks
            : [{ ordinal: 0, kind: "text", text: input.content }];
        chunks.push(...normalizedChunks.map((chunk) => ({
        ...
        media.push(...(input.media ?? []).map((entry) => {
            ...
            if (accessMode !== "public" && !entry.storageKey) {
                throw new Error("Private or expiring media must be copied to private blob storage; provider URLs may be kept only as provenanceUrl");
            }
            return {
        ...
        edges.push(...(input.edges ?? []).map((edge) => {
    ...
    try {
        await getDb().transaction(async (tx) => {
            ...
            if (chunks.length)
                await tx.insert(schema.contextChunks).values(chunks);
            if (media.length)
                await tx.insert(schema.contextMedia).values(media);
            if (edges.length)
                await tx.insert(schema.contextEdges).values(edges);
            ...
            const [total] = await tx
                .select({ value: count() })
                .from(schema.contextItems)
                .where(and(eq(schema.contextItems.sourceId, batch.sourceId), eq(schema.contextItems.status, "active")));
            const [restricted] = await tx
                .select({ value: count() })
                .from(schema.contextItems)
                .where(and(eq(schema.contextItems.sourceId, batch.sourceId), eq(schema.contextItems.upstreamAccess, "restricted")));
            await tx
    ...
    if (chunks.length) {
        await enqueueContextRebuildJob({
            ...
            itemIds: [...new Set(chunks.map((chunk) => chunk.itemId))],
        });
    }
    const mediaProjection = getCreativeContext().projections?.media;
    if (mediaProjection) {
        const eager = media.slice(0, 5);
        const queued = media.slice(5).map((row) => row.id);
...
async function upsertSourceInventoryAttempt(input, allowUniqueConflictRetry) {
    const access = await assertAccess("creative-context-source", input.sourceId, "editor");
    ...
    const existing = externalIds.length
        ? await getDb()
            .select()
            .from(schema.contextItems)
            .where(and(eq(schema.contextItems.sourceId, input.sourceId), inArray(schema.contextItems.externalId, externalIds)))
        : [];
    ...
    for (const item of input.items) {
        ...
        if (prior) {
            const upstreamAccess = item.upstreamAccess ?? access.resource.upstreamAccess ?? "unknown";
            const newlyRestricted = upstreamAccess === "restricted" &&
        ...
        const contentHash = await sha256(stringifyJson({
            inventory: true,
    ...
    try {
        await getDb().transaction(async (tx) => {
            ...
            const [restricted] = await tx
                .select({ value: count() })
                .from(schema.contextItems)
                .where(and(eq(schema.contextItems.sourceId, input.sourceId), eq(schema.contextItems.upstreamAccess, "restricted")));
            await tx
                .update(schema.contextSources)
                .set({
                itemCount: Number(total?.value ?? 0),
...
export async function reconcileSourceInventory(input) {
    await assertAccess("creative-context-source", input.sourceId, "editor", undefined, { skipResourceBody: true });
    const { getDb, schema } = getCreativeContext();
    ...
    const removedIds = rows
        .filter((row) => !present.has(row.externalId) &&
    ...
    await getDb().transaction(async (tx) => {
        for (let index = 0; index < removedIds.length; index += 500) {
            await tx
                ...
                .where(inArray(schema.contextItems.id, removedIds.slice(index, index + 500)));
        }
        for (let index = 0; index < restoredIds.length; index += 500) {
            await tx
                ...
                .where(inArray(schema.contextItems.id, restoredIds.slice(index, index + 500)));
        }
        await tx
            .update(schema.contextSources)
            .set({
            itemCount: present.size,
            restrictedItemCount: rows.filter((row) => present.has(row.externalId) && row.upstreamAccess === "restricted").length,
            lastSyncedAt: timestamp,
            updatedAt: timestamp,
        })
            .where(eq(schema.contextSources.id, input.sourceId));
...
function searchableItemStatuses(statuses) {
    if (!statuses?.length)
        return ["active"];
    const safe = statuses.filter((status) => status === "active" || status === "deprecated");
    if (safe.length !== statuses.length) {
        throw new Error("Creative context search cannot include removed items");
    }
    return [...new Set(safe)];
}
async function accessiblePackVersionIds(packId) {
    ...
    return members.map((member) => member.itemVersionId);
}
async function accessibleContextVersionIds(contextId) {
    ...
    return rows.flatMap((row) => typeof row.itemVersionId === "string" ? [row.itemVersionId] : []);
}
async function accessibleContextMembershipMetadata(contextId) {
    await assertAccess("creative-context", contextId, "viewer", undefined, {
        skipResourceBody: true,
    });
    const { getDb, schema } = getCreativeContext();
    const rows = await getDb()
        ...
        .from(schema.creativeContextMemberships)
        .where(and(eq(schema.creativeContextMemberships.contextId, contextId), eq(schema.creativeContextMemberships.status, "active")));
    return new Map(rows.flatMap((row) => typeof row.itemVersionId === "string"
        ? [
            [
                row.itemVersionId,
...
export async function listAccessibleSearchDocuments(input) {
    const { getDb, schema } = getCreativeContext();
    const packVersionIds = input.packId
        ? await accessiblePackVersionIds(input.packId)
        : null;
    const contextVersionIds = input.contextId
    ...
    const contextMemberships = input.contextId
        ...
        : null;
    if ((packVersionIds && !packVersionIds.length) ||
        (contextVersionIds && !contextVersionIds.length))
        return [];
    const sourceAccess = and(accessFilter(schema.contextSources, schema.contextSourceShares), ne(schema.contextSources.upstreamAccess, "restricted"), ne(schema.contextSources.status, "archived"));
    const filters = [
        ...(input.contextId
        ...
        ...(packVersionIds
            ...
            : [
                eq(schema.contextItems.currentVersionId, schema.contextChunks.itemVersionId),
            ]),
    ...
    while (rows.length < rowLimit) {
        const page = await getDb()
            ...
            body: sql `substr(${schema.contextChunks.text}, 1, 12000)`,
            ...
            .innerJoin(schema.contextItems, eq(schema.contextItems.id, schema.contextChunks.itemId))
            .innerJoin(schema.contextSources, eq(schema.contextSources.id, schema.contextItems.sourceId))
            .innerJoin(schema.contextItemVersions, eq(schema.contextItemVersions.id, schema.contextChunks.itemVersionId))
            .leftJoin(schema.creativeContextPublishedSnapshots, and(eq(schema.creativeContextPublishedSnapshots.itemId, schema.contextItems.id), eq(schema.creativeContextPublishedSnapshots.itemVersionId, schema.contextChunks.itemVersionId), eq(schema.creativeContextPublishedSnapshots.sourceId, schema.contextItems.sourceId)))
            .where(and(...filters, ...(afterChunkId ? [gt(schema.contextChunks.id, afterChunkId)] : [])))
            .orderBy(asc(schema.contextChunks.id))
            .limit(1_000);
        for (const row of page) {
            const tags = parseJson(row.tags, []);
            const colors = parseJson(row.colors, []);
            if (!(input.tags?.every((tag) => tags.includes(tag)) ?? true) ||
    ...
    const actor = requireActor();
    const versionIds = [
        ...new Set(documents.map((row) => row.itemVersionId)),
    ];
    const [reuseRows, feedbackRows] = await Promise.all([
        getDb()
            .select({ itemVersionId: schema.contextPackMembers.itemVersionId })
            .from(schema.contextPackMembers)
            .where(and(inArray(schema.contextPackMembers.itemVersionId, versionIds), eq(schema.contextPackMembers.ownerEmail, actor.ownerEmail))),
        getDb()
            .select({ itemVersionId: schema.contextFeedback.itemVersionId })
            .from(schema.contextFeedback)
            .where(and(inArray(schema.contextFeedback.itemVersionId, versionIds), eq(schema.contextFeedback.ownerEmail, actor.ownerEmail), eq(schema.contextFeedback.signal, "helpful"))),
    ]);
    const reuseCounts = new Map();
    const helpfulCounts = new Map();
    for (const row of reuseRows) {
        reuseCounts.set(row.itemVersionId, (reuseCounts.get(row.itemVersionId) ?? 0) + 1);
    }
    for (const row of feedbackRows) {
        helpfulCounts.set(row.itemVersionId, (helpfulCounts.get(row.itemVersionId) ?? 0) + 1);
    }
...
export async function listAccessibleLexicalCandidates(input) {
    const { getDb, schema } = getCreativeContext();
    ...
    if (!terms.length && matchMode !== "regex")
        return { results: [] };
    ...
    const likeClauses = matchTerms.map((term) => {
        ...
        return or(sql `lower(${schema.contextItems.title}) like ${pattern} escape '\\'`, sql `lower(${schema.contextChunks.text}) like ${pattern} escape '\\'`);
    });
    const sourceAccess = and(accessFilter(schema.contextSources, schema.contextSourceShares), ne(schema.contextSources.upstreamAccess, "restricted"), ne(schema.contextSources.status, "archived"));
    const filters = [
        ...(input.contextId
    ...
    if (matchMode !== "regex") {
        if (matchMode === "phrase") {
            ...
            filters.push(or(sql `lower(${schema.contextItems.title}) like ${phrasePattern} escape '\\'`, sql `lower(${schema.contextChunks.text}) like ${phrasePattern} escape '\\'`));
        }
        else {
            filters.push(matchMode === "allTerms" ? and(...likeClauses) : or(...likeClauses));
        }
    }
    if (input.sourceIds?.length) {
        filters.push(inArray(schema.contextSources.id, input.sourceIds));
    ...
    while (true) {
        ...
        for (const row of rows) {
            const tags = new Set(parseJson(row.tags, []));
            const colors = new Set(parseJson(row.colors, []));
            if (!(input.tags?.every((tag) => tags.has(tag)) ?? true) ||
                !(input.colors?.every((color) => colors.has(color)) ?? true) ||
                !matchesCreativeSearchMode(`${row.title}\n${row.summary ?? ""}\n${row.body}`, input.query, matchMode)) {
                continue;
            }
            scored.push({
                itemId: row.itemId,
    ...
    return {
        results: page.slice(0, input.limit),
        nextCursor: nextOffsetCursor(offset, input.limit, hasMore),
    };
}
export async function getCreativeContextItem(itemId, itemVersionId) {
    const { getDb, schema } = getCreativeContext();
    ...
    const itemRows = await getDb()
        ...
        .from(schema.contextItems)
        .innerJoin(schema.contextSources, eq(schema.contextSources.id, schema.contextItems.sourceId))
        .leftJoin(schema.creativeContextMemberships, and(eq(schema.creativeContextMemberships.publishedItemId, schema.contextItems.id), eq(schema.creativeContextMemberships.publishedItemVersionId, requestedVersionId), eq(schema.creativeContextMemberships.status, "active")))
        .leftJoin(schema.creativeContexts, eq(schema.creativeContexts.id, schema.creativeContextMemberships.contextId))
        .leftJoin(schema.contextPackMembers, and(eq(schema.contextPackMembers.itemId, schema.contextItems.id), eq(schema.contextPackMembers.itemVersionId, requestedVersionId)))
        .leftJoin(schema.contextPacks, eq(schema.contextPacks.id, schema.contextPackMembers.packId))
        .leftJoin(schema.creativeContextPublishedSnapshots, and(eq(schema.creativeContextPublishedSnapshots.itemId, schema.contextItems.id), eq(schema.creativeContextPublishedSnapshots.itemVersionId, requestedVersionId), eq(schema.creativeContextPublishedSnapshots.sourceId, schema.contextItems.sourceId)))
        .where(and(eq(schema.contextItems.id, itemId), or(and(accessFilter(schema.contextSources, schema.contextSourceShares), ne(schema.contextSources.upstreamAccess, "restricted"), ne(schema.contextSources.status, "archived")), and(isNotNull(schema.creativeContextPublishedSnapshots.id), accessFilter(schema.contextPacks, schema.contextPackShares)), accessFilter(schema.creativeContexts, schema.creativeContextShares)), eq(schema.contextItems.curationStatus, "included"), ne(schema.contextItems.curationRank, "ignored"), eq(schema.contextItems.status, "active")))
        .limit(1);
    ...
    const versionRows = await getDb()
        .select()
        .from(schema.contextItemVersions)
        .where(and(eq(schema.contextItemVersions.id, targetVersionId), eq(schema.contextItemVersions.itemId, itemId)))
        .limit(1);
    if (!versionRows[0])
        return null;
...
export async function appendMediaEnrichmentVersion(input) {
    ...
    if (!itemId)
        throw new Error("Creative context media was not found");
    const detail = await getCreativeContextItem(itemId);
    if (!detail)
        throw new Error("Creative context media is not accessible");
    const target = detail.media.find((media) => media.id === input.mediaId ||
        media.metadata.__creativeContextEnrichmentSourceMediaId ===
            input.mediaId ||
        media.metadata.__creativeContextPreviousMediaId === input.mediaId);
    if (!target) {
        throw new Error("Creative context media enrichment can only append from the current item version");
    }
    if (target.caption === input.caption &&
    ...
    const sourceContentHash = mediaEnrichmentSourceContentHash(detail.version.metadata, detail.version.contentHash);
    const media = detail.media.map((entry) => {
        ...
        return {
            ...
            metadata: {
                ...entry.metadata,
                __creativeContextEnrichmentSourceMediaId: entry.metadata.__creativeContextEnrichmentSourceMediaId ?? entry.id,
    ...
    const canonicalMedia = media.map((entry) => {
        const { __creativeContextEnrichmentSourceMediaId: _sourceMediaId, __creativeContextPreviousMediaId: _previousMediaId, ...metadata } = entry.metadata;
        return { ...entry, metadata };
    });
    const derivedContentHash = await sha256(stringifyJson({
        sourceContentHash,
        ...
        items: [
            {
                ...
                media,
                edges: detail.edges
                    .filter((edge) => edge.relation !== "revision-of" ||
    ...
    const enrichedMedia = appended?.media.find((entry) => entry.metadata.__creativeContextPreviousMediaId === target.id);
...
export async function getCreativeContextItemByExternalId(input) {
    if (Boolean(input.itemId) !== Boolean(input.itemVersionId))
        return null;
    if (input.itemId && input.itemVersionId) {
        const pinned = await getCreativeContextItem(input.itemId, input.itemVersionId);
    ...
    const itemRows = await getDb()
        ...
        .from(schema.contextItems)
        .where(and(eq(schema.contextItems.sourceId, input.sourceId), eq(schema.contextItems.externalId, input.externalId)))
        .limit(1);
    const itemId = itemRows[0]?.id;
    if (!itemId)
        return null;
    if (!input.sourceVersion)
        return getCreativeContextItem(itemId);
    const versionRows = await getDb()
        .select({ id: schema.contextItemVersions.id })
        .from(schema.contextItemVersions)
        .where(and(eq(schema.contextItemVersions.itemId, itemId), eq(schema.contextItemVersions.sourceVersion, input.sourceVersion)))
...
export async function createEmbeddingSet(input) {
    ...
    if (actor.orgId) {
        if (!(await currentRequestUserIsOrgAdmin(actor.orgId))) {
            throw new Error("Only organization owners or admins can activate an organization embedding family.");
        }
    }
    const row = {
        id: newId("cces"),
    ...
    await getDb().transaction(async (tx) => {
        await tx
            ...
            .set({ status: "retired" })
            .where(and(actor.orgId
            ? eq(schema.embeddingSets.orgId, actor.orgId)
            : eq(schema.embeddingSets.ownerEmail, actor.ownerEmail), eq(schema.embeddingSets.status, "active")));
        await tx.insert(schema.embeddingSets).values(row);
    });
    return { ...row, metadata: parseJson(row.metadata, {}) };
}
export async function getActiveEmbeddingSet(input = {}) {
    const { getDb, schema } = getCreativeContext();
    const actor = requireActor();
    const filters = [
        actor.orgId
    ...
    if (input.family)
        filters.push(eq(schema.embeddingSets.family, input.family));
    if (input.model)
        filters.push(eq(schema.embeddingSets.model, input.model));
    if (input.version)
    ...
    const rows = await getDb()
        ...
        .where(and(...filters))
        .orderBy(desc(schema.embeddingSets.createdAt), desc(schema.embeddingSets.id))
        .limit(1);
    const row = rows[0];
    return row
        ? {
...
export async function recordEmbeddingMetadata(input) {
    ...
    if (input.chunkId) {
        ...
        if (!chunk)
            throw new Error("Embedding chunk must belong to item version");
    }
    ...
    const sets = await getDb()
        .select()
        .from(schema.embeddingSets)
        .where(and(eq(schema.embeddingSets.id, input.embeddingSetId), actor.orgId
        ? eq(schema.embeddingSets.orgId, actor.orgId)
        : eq(schema.embeddingSets.ownerEmail, actor.ownerEmail)))
        .limit(1);
    const set = sets[0];
    ...
    const existing = await getDb()
        ...
        .where(and(eq(schema.embeddings.embeddingSetId, input.embeddingSetId), eq(schema.embeddings.targetType, targetType), eq(schema.embeddings.targetId, targetId)))
        .limit(1);
    const row = {
...
export async function listEmbeddingMetadata(input) {
    const { getDb, schema } = getCreativeContext();
    if (input.itemVersionIds && !input.itemVersionIds.length)
        return [];
    const packVersionIds = input.packId
        ? await accessiblePackVersionIds(input.packId)
        : null;
    ...
    const filters = [
        ...
        ...(packVersionIds
            ...
            : [
                eq(schema.contextItems.currentVersionId, schema.embeddings.itemVersionId),
            ]),
    ...
    if (input.itemVersionIds) {
        filters.push(inArray(schema.embeddings.itemVersionId, input.itemVersionIds));
    }
    if (input.sourceIds?.length) {
        filters.push(inArray(schema.contextSources.id, input.sourceIds));
    }
    ...
    const rows = await getDb()
        ...
        .from(schema.embeddings)
        .innerJoin(schema.contextItems, eq(schema.contextItems.id, schema.embeddings.itemId))
        .innerJoin(schema.contextSources, eq(schema.contextSources.id, schema.contextItems.sourceId))
        .where(and(...filters));
    return rows
        .filter((row) => {
        const tags = parseJson(row.tags, []);
        const colors = parseJson(row.colors, []);
        return ((input.tags?.every((tag) => tags.includes(tag)) ?? true) &&
            (input.colors?.every((color) => colors.includes(color)) ?? true));
    })
...
export async function reviewContextItems(input) {
    const { getDb, schema } = getCreativeContext();
    await assertAccess("creative-context-source", input.sourceId, input.operation === "list" ? "viewer" : "editor", undefined, { skipResourceBody: true });
    const pendingFilter = and(eq(schema.contextItems.sourceId, input.sourceId), eq(schema.contextItems.upstreamAccess, "restricted"), eq(schema.contextItems.curationStatus, "review"));
    if (input.operation !== "list") {
        const itemIds = Array.from(new Set(input.itemIds ?? []));
        if (!itemIds.length)
            throw new Error("itemIds are required for review");
        const eligibleFilter = input.operation === "approve" || input.operation === "exclude"
            ? and(eq(schema.contextItems.sourceId, input.sourceId), eq(schema.contextItems.upstreamAccess, "restricted"), inArray(schema.contextItems.curationStatus, [
                "review",
                "included",
                "excluded",
            ]), inArray(schema.contextItems.id, itemIds))
            : and(eq(schema.contextItems.sourceId, input.sourceId), inArray(schema.contextItems.id, itemIds));
        const eligible = await getDb()
            .select({ id: schema.contextItems.id })
            .from(schema.contextItems)
            .where(eligibleFilter);
        if (eligible.length !== itemIds.length) {
            throw new Error("All curated items must belong to the accessible source and be eligible for the operation");
        }
        const valuesByOperation = {
    ...
    const rows = await getDb()
        ...
        .from(schema.contextItems)
        .where(input.queue === "all"
        ? and(eq(schema.contextItems.sourceId, input.sourceId), ne(schema.contextItems.status, "deleted"))
        : pendingFilter)
