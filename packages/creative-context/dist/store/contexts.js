...
function creativeContextAccessRole(role) {
...
export async function getCreativeContextById(contextId) {
    ...
    return mapContext(access.resource, Number(membershipCount?.value ?? 0), creativeContextAccessRole(access.role));
}
...
export async function listCreativeContexts(input) {
    ...
    return {
        contexts: page.flatMap((row) => {
            ...
            return access
                ? [
                    mapContext(row, byContext.get(row.id) ?? 0, creativeContextAccessRole(access.role)),
                ]
                : [];
        }),
