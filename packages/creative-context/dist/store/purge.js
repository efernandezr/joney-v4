...
import { resourceDeleteByPath, sharedResourceOwner, } from "@agent-native/core/resources/store";
...
export async function purgeContextSourceArtifacts(sourceId) {
    ...
    if (!source)
        throw new Error("Creative context source not found");
    ...
    const derivedSubmissions = await getDb()
        ...
        .where(like(schema.creativeContextSubmissions.artifactKey, `${sourceId}:%`));
    const derivedSubmissionIds = derivedSubmissions.map((row) => String(row.id));
    const publishedCopies = derivedSubmissionIds.length
        ? await getDb()
            ...
            .where(inArray(schema.creativeContextPublishedSnapshots.submissionId, derivedSubmissionIds))
        : [];
    const stagedChildIds = derivedSubmissions.flatMap((row) => {
        const metadata = parseJson(row.privateMetadata, {});
        return Array.isArray(metadata.stagedChildren)
            ? metadata.stagedChildren.flatMap((child) => child &&
                typeof child === "object" &&
    ...
    const candidateItemIds = [
        ...originalItems.map((item) => String(item.id)),
        ...derivedSubmissions.flatMap((row) => [row.stagingItemId, row.publishedItemId].filter((value) => typeof value === "string")),
        ...publishedCopies.map((row) => String(row.itemId)),
        ...stagedChildIds,
    ];
    ...
    if (!itemIds.length)
        return { sourceId, purgedItems: 0, purgedBlobs: 0 };
    const affectedSourceIds = [
        ...new Set([sourceId, ...items.map((item) => String(item.sourceId))]),
    ];
    ...
    const affectedMembershipIds = [
        ...new Set([
            ...referencedMemberships.map((row) => String(row.id)),
            ...derivedSubmissions.map((row) => String(row.membershipId)),
        ]),
    ];
    const [chunks, media, embeddings, derivedEdges, evidence, packMemberships, suggestions, generationRows,] = await Promise.all([
        getDb()
    ...
    const dnaVersionIds = [
        ...new Set(evidence.map((row) => String(row.dnaVersionId))),
    ];
    const affectedProfiles = dnaVersionIds.length
        ? await getDb()
            ...
            .innerJoin(schema.brandProfiles, eq(schema.brandProfiles.id, schema.brandDnaVersions.profileId))
            .where(inArray(schema.brandDnaVersions.id, dnaVersionIds))
        : [];
    const promotedLayouts = await Promise.all(suggestions
        .filter((suggestion) => suggestion.status === "promoted")
        .map(async (suggestion) => {
        const payload = parseJson(suggestion.payload, {});
        return {
            suggestionId: suggestion.id,
            projectionItemId: await resolveLayoutProjectionItemId(suggestion.id, payload.projectionItemId),
        };
    }));
    for (const layout of promotedLayouts) {
        await projections?.layoutTemplate?.demote(layout);
    }
    const cloneBlobRefs = derivedSubmissions.flatMap((row) => {
        const metadata = parseJson(row.privateMetadata, {});
    ...
    for (const reference of [
        ...versions.map((version) => version.rawSnapshotBlobRef),
        ...media.map((entry) => entry.storageKey),
        ...cloneBlobRefs,
    ]) {
    ...
    await deletePostgresFtsDocuments(getDbExec(), chunks.map((chunk) => chunk.id)).catch(() => 0);
    await getDb().transaction(async (tx) => {
        if (affectedMembershipIds.length) {
            await tx
                ...
                .where(inArray(schema.creativeContextMemberships.id, affectedMembershipIds));
        }
        await tx
            .delete(schema.creativeContextPublishedSnapshots)
            .where(derivedSubmissionIds.length
            ? or(inArray(schema.creativeContextPublishedSnapshots.itemId, itemIds), inArray(schema.creativeContextPublishedSnapshots.submissionId, derivedSubmissionIds))
        ...
        if (derivedSubmissionIds.length) {
            await tx
                ...
                .where(inArray(schema.creativeContextSubmissions.id, derivedSubmissionIds));
        }
        if (derivedEdges.length) {
            await tx
                .update(schema.contextItems)
                .set({ status: "deprecated", updatedAt: nowIso() })
                .where(inArray(schema.contextItems.id, derivedEdges.map((edge) => edge.fromItemId)));
        }
        const projectionItemIds = promotedLayouts.flatMap((layout) => layout.projectionItemId ? [layout.projectionItemId] : []);
        ...
        for (const record of generationRows) {
            const provenance = parseJson(record.elementProvenance, []);
            let changed = false;
            const redacted = provenance.map((entry) => {
                ...
                changed = true;
                const { itemId: _itemId, itemVersionId: _versionId, ...summary } = entry;
        ...
        await tx
            ...
            .where(inArray(schema.contextPackMembers.itemId, itemIds));
        const affectedPackIds = [
            ...new Set(packMemberships.map((row) => String(row.packId))),
        ];
        ...
        await tx
            ...
            .where(inArray(schema.brandDnaEvidence.itemId, itemIds));
        const affectedProfileIds = Array.from(new Set(affectedProfiles.map((row) => row.profileId)));
        ...
        await tx
            ...
            .where(or(inArray(schema.contextEdges.fromItemId, itemIds), inArray(schema.contextEdges.toItemId, itemIds)));
        await tx
    ...
    for (const affectedSourceId of affectedSourceIds) {
        ...
        await getDb()
            ...
            restrictedItemCount: remaining.filter((item) => item.upstreamAccess !== "available").length,
            updatedAt: nowIso(),
        })
            .where(eq(schema.contextSources.id, affectedSourceId));
    }
    const profileIds = [
        ...new Set(affectedProfiles.map((row) => row.profileId)),
    ];
    for (const profile of affectedProfiles) {
        const owner = profile.visibility === "org" && profile.orgId
            ? sharedResourceOwner(profile.orgId)
            : profile.ownerEmail;
        await resourceDeleteByPath(owner, "context/brand-context.md").catch(() => false);
    }
    const remainingSources = profileIds.length
        ? await getDb()
            ...
            .from(schema.contextSources)
            .where(and(accessFilter(schema.contextSources, schema.contextSourceShares), eq(schema.contextSources.status, "active")))
        : [];
    let dnaRecomputeJobs = 0;
    for (const profileId of profileIds) {
        const recomputeSource = remainingSources.find((candidate) => candidate.id !== sourceId &&
            parseJson(candidate.config, {}).profileId ===
                profileId);
        if (recomputeSource) {
            await createJob({
