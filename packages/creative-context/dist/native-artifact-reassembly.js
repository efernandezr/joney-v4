...
export async function reassembleNativeCreativeArtifact(input) {
    ...
    const headResources = new Set();
    const visit = async (detail, depth) => {
        ...
        if (!artifact ||
            ...
            throw new Error(`Creative context item ${detail.item.id} is not a ${input.format} native artifact.`);
        }
        if (detail.item.mimeType !== "text/html" ||
            !isTrustedCompilerItem(detail)) {
            throw new Error(`Creative context item ${detail.item.id} was not produced by a trusted native artifact compiler.`);
        }
        ...
        for (const child of [...artifact.manifest.children].sort((left, right) => left.zOrder - right.zOrder)) {
            const pinnedEdge = detail.edges.find((edge) => edge.relation === "contains-native-child" &&
                edge.toExternalId === child.externalId);
            if ((pinnedEdge?.toItemId && !pinnedEdge.toItemVersionId) ||
                (!pinnedEdge?.toItemId && pinnedEdge?.toItemVersionId)) {
                throw new Error(`Native creative artifact child ${child.externalId} has an incomplete pinned version reference.`);
            }
            if (!pinnedEdge?.toItemId &&
                !pinnedEdge?.toItemVersionId &&
                !detail.version.sourceVersion) {
                throw new Error(`Native creative artifact child ${child.externalId} has no immutable version reference.`);
            }
            ...
            if (!childDetail) {
                throw new Error(`Native creative artifact child ${child.externalId} is unavailable at the pinned source version.`);
            }
            const compiledChild = await visit(childDetail, depth + 1);
            collectHeadResources(compiledChild.html, headResources);
            html = fillGeneratedChildPlaceholder(html, child, extractBody(compiledChild.html, compiledChild.artifact.format), compiledChild.artifact.format);
...
export function validateCompiledNativeHtml(html, artifact) {
    if (artifact.format === "design-html" &&
        (!/^\s*<!doctype html>/i.test(html) || !/<html(?:\s|>)/i.test(html))) {
        throw new Error("Native creative artifact must be a complete HTML document.");
    }
    if (/<\s*(?:script|iframe|object|embed|base|form|input|button|textarea|select|video|audio|source)\b/i.test(html) ||
    ...
    if (/\s(?:x-|hx-|ng-|v-)[a-z0-9_:.@-]*\s*=/i.test(html) ||
        /\s[@:][^\s=/>]+\s*=/.test(html)) {
        throw new Error("Native creative artifact contains executable framework attributes.");
    }
    if (/\s(?:srcset|lowsrc|background|poster|cite|ping|longdesc|usemap|formaction|action|manifest|xlink:href)\s*=/i.test(html) ||
        /(?:-webkit-)?image-set\s*\(/i.test(html)) {
        throw new Error("Native creative artifact contains an unsupported URL-bearing construct.");
    }
    for (const match of html.matchAll(/<\/?\s*([a-z][a-z0-9-]*)\b/gi)) {
        const tag = match[1]?.toLowerCase();
        if (tag && !ALLOWED_TAGS.has(tag)) {
            throw new Error(`Native creative artifact contains unsupported <${tag}> markup.`);
        }
    }
    const urlAttributes = [
    ...
    if (urlAttributes.length !== attributeCount) {
        throw new Error("Native creative artifact contains an invalid URL attribute.");
    }
    ...
    for (const match of html.matchAll(/<style(?:\s[^>]*)?>([\s\S]*?)<\/style>|\sstyle\s*=\s*(["'])(.*?)\2/gi)) {
        const css = match[1] ?? match[3] ?? "";
        if (/(?:https?:|data:|javascript:|(?:^|[\s("'])\/\/)/i.test(css)) {
            throw new Error("Native creative artifact CSS contains an external or executable URL.");
        }
    }
    const usedAssetRefs = collectNativeAssetUrls(html);
    const declaredAssetRefs = [...new Set(artifact.assetRefs ?? [])].map(canonicalMediaUrl);
    for (const assetRef of declaredAssetRefs)
        assertSafeMediaUrl(assetRef);
    if (usedAssetRefs.length !== declaredAssetRefs.length ||
        usedAssetRefs.some((value) => !declaredAssetRefs.includes(value))) {
        throw new Error("Native creative artifact code does not match its declared asset references.");
    }
    const placeholders = [
        ...html.matchAll(/data-creative-context-child\s*=\s*"([^"]+)"/g),
    ].map((match) => match[1]);
    const expected = (artifact.manifest?.children ?? []).map((child) => escapeAttribute(child.externalId));
    const declaredChildren = artifact.childExternalIds ?? [];
    const manifestChildren = (artifact.manifest?.children ?? []).map((child) => child.externalId);
    if (placeholders.length !== expected.length ||
        placeholders.some((value, index) => value !== expected[index]) ||
        new Set(placeholders).size !== placeholders.length) {
        throw new Error("Native creative artifact code does not match its child manifest.");
    }
    if (declaredChildren.length !== manifestChildren.length ||
        declaredChildren.some((value, index) => value !== manifestChildren[index])) {
        throw new Error("Native creative artifact child ids do not match its child manifest.");
    }
}
function assertSafeMediaUrl(value) {
    const canonical = canonicalMediaUrl(value);
    if (!canonical.startsWith(SAFE_MEDIA_ROUTE)) {
        throw new Error("Native creative artifact assets must use the private relative media route.");
    }
    const url = new URL(canonical, "https://creative-context.invalid");
    if (url.origin !== "https://creative-context.invalid" ||
...
function fillGeneratedChildPlaceholder(html, child, childBody, format) {
    ...
    if (openStart < 0 ||
        ...
        throw new Error(`Native creative artifact child placeholder ${externalId} is malformed.`);
    }
    const opening = format === "slides-html"
        ? "<div>"
...
function collectNativeAssetUrls(html) {
    const values = [
        ...[...html.matchAll(/\s(?:src|href)\s*=\s*(["'])(.*?)\1/gi)].map((match) => match[2] ?? ""),
        ...[...html.matchAll(/url\(\s*(["']?)(.*?)\1\s*\)/gi)].map((match) => match[2] ?? ""),
    ]
        .filter(Boolean)
...
function extractBody(html, format) {
...
function collectHeadResources(html, resources) {
    const match = html.match(/<head(?:\s[^>]*)?>([\s\S]*?)<\/head>/i);
    if (!match?.[1])
        return;
    for (const entry of match[1].matchAll(/<style(?:\s[^>]*)?>[\s\S]*?<\/style>|<link\s[^>]*rel=["']stylesheet["'][^>]*\/?>/gi)) {
        if (entry[0])
            resources.add(entry[0]);
    }
}
function insertHeadResources(html, resources) {
...
function assertReassembledSize(html) {
...
//# sourceMappingURL=native-artifact-reassembly.js.map