...
import { z } from "zod";
import { reassembleNativeCreativeArtifact, validateCompiledNativeHtml, } from "../native-artifact-reassembly.js";
import { nativeCreativeArtifactFromMetadata, } from "../native-artifact.js";
import { sanitizePublicString, serializePublicContextDetail, } from "../server/public-serialization.js";
import { ensureContextItemHydration } from "../server/retrieval.js";
import { getCreativeContextItem, getCreativeContextItemByExternalId, } from "../store/index.js";
import { delimitUntrustedMetadata, delimitUntrustedReference, UNTRUSTED_REFERENCE_ROLE, } from "../untrusted-reference.js";
const MAX_PUBLIC_NATIVE_CODE_BYTES = 128 * 1024;
export default defineAction({
    description: "Get one accessible curated creative-context item at a pinned immutable version.",
    schema: z.object({
    ...
    run: async (args) => {
        const context = await getCreativeContextItem(args.itemId, args.itemVersionId);
        if (!context)
            throw new Error("Context item not found or not accessible");
        const publicContext = serializePublicContextDetail(context);
        const nativeArtifact = nativeCreativeArtifactFromMetadata(context.version.metadata);
        const nativeCode = nativeArtifact
            ? await publicNativeCode(context, nativeArtifact)
            : null;
        const nativeCodeOversized = Boolean(nativeCode && "oversized" in nativeCode && nativeCode.oversized);
        return {
            ...publicContext,
            ...
            item: {
                ...
                title: delimitUntrustedReference(sanitizePublicString(context.item.title)),
                provenance: delimitUntrustedMetadata(publicContext.item.provenance),
            },
            version: {
                ...publicContext.version,
                dataRole: UNTRUSTED_REFERENCE_ROLE,
                title: delimitUntrustedReference(sanitizePublicString(context.version.title)),
                content: delimitUntrustedReference(nativeCodeOversized
                ...
                summary: context.version.summary
                    ? delimitUntrustedReference(sanitizePublicString(context.version.summary))
                    : null,
                ...
                parseError: context.version.parseError
                    ? delimitUntrustedReference(sanitizePublicString(context.version.parseError))
                    : null,
...
async function publicNativeCode(context, artifact) {
    try {
        ...
        return {
            ...
            retrieval: nativeCodeRetrieval(context, artifact),
            instruction: "Use the exact clone action for the complete artifact. For a manifest artifact, inspect individually pinned parts with get-context-item; never concatenate a truncated HTML fragment.",
        };
    }
    catch {
        return null;
...
function nativeCodeRetrieval(context, artifact) {
    return {
        ...
        cloneAction: artifact.app === "slides"
            ? "clone-context-slide"
            : "clone-creative-context-design",
        parts: (artifact.manifest?.children ?? []).map((child) => {
            const edge = context.edges.find((candidate) => candidate.relation === "contains-native-child" &&
...
//# sourceMappingURL=get-creative-context.js.map