...
const importableSourceKinds = CREATIVE_CONTEXT_SOURCE_KINDS.filter((kind) => kind !== "native-app");
import { archiveContextSource, createContextSource, deleteContextSource, previewContextSourcePromotion, promoteContextSource, restoreContextSource, toSourceSummary, updateContextSource, } from "../store/index.js";
...
export default defineAction({
    description: "Create or update a private source, archive/restore it, preview a deliberate org promotion, promote after confirming container and count, or tombstone it and enqueue purge.",
    schema,
    agentInputSchema,
    needsApproval: (args) => args.operation === "delete" || args.operation === "promote",
    publicAgent: {
        expose: true,
        readOnly: false,
        requiresAuth: true,
    ...
    run: async (args) => {
        ...
        if (args.operation === "promote") {
            const source = await promoteContextSource(args.sourceId, args.confirmation);
            return { source: toSourceSummary(source), deleted: false };
        }
...
//# sourceMappingURL=manage-context-source.js.map