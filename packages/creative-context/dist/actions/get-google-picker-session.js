...
export default defineAction({
    description: "Return a short-lived Google Picker session for the signed-in Library UI.",
    schema: z.object({
    ...
    run: async ({ connectionId }) => {
        ...
        const [oauth, apiKey, appId] = await Promise.all([
            resolveProviderApiOAuthAccessToken({ provider: "google_drive", connectionId }, {
        ...
        if (!apiKey || !appId) {
            throw new Error("Google Picker is not configured. Set GOOGLE_PICKER_API_KEY and GOOGLE_PICKER_APP_ID.");
        }
        return {
...
//# sourceMappingURL=get-google-picker-session.js.map