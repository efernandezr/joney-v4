...
import { parseNativeCreativeArtifactKey, resolveNativeCreativeResourceUpdateStatuses, } from "../server/native-resource-capture.js";
...
export default defineAction({
    ...
    run: async (args) => {
        ...
        const references = result.memberships.flatMap((membership) => {
            ...
            return [
                {
                    key: membership.id,
                    ...reference,
                    publishedSourceModifiedAt: membership.publishedItem.sourceModifiedAt ?? null,
                },
            ];
        });
        const statuses = await resolveNativeCreativeResourceUpdateStatuses(references);
        return {
            ...result,
...
//# sourceMappingURL=list-context-memberships.js.map